...
get_coding <- function(survey_sheet, survey_type) {
  read_sheet(codebook, sheet = survey_sheet, na = c("", "NA")) |>
    ...
    select(variable_name, contains("construct"), question_type, reverse_coded,
           response_options)
}
...
survey_items <- survey_coding |>
  ...
  select(survey_type, variable = variable_name, variable_order,
         contains("construct"), question_type, reverse_coded, values = values_str)
...
survey_table <- item_metadata$table("survey_items")
