...
tasks <- task_data_irt |> unnest(data) |> distinct(task_id, item_task)
mods_best_overlap |> select(item_task = task, itemtype, invariance) |>
  left_join(tasks) |> relocate(task_id, .after = item_task)
...
scores_labeled <- scores_best |>
  ...
  select(item_task = task, run_id, metric_type, metric_value = ability, model_class, model)
  # left_join(run_info)
