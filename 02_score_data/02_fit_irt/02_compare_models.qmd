...
ggsave(here("02_scoring_outputs/plots/bic_comparison.png"), width = 9, height = 6)
...
mods_lang_rxx <- mods_objs |>
  filter(model_set == "by_language") |>
  semi_join(mods_best_configural, by = "task") |>
  mutate(site = subset |> fct_recode("pilot_mpieva_de" = "de-DE",
                                     "pilot_uniandes_co" = "es-CO",
                                     "pilot_western_ca" = "en-US")) |>
  mutate(rxx = map_dbl(mod, marginal_rxx),
         invariance = "by_language") |>
  select(site, task, itemtype, nfact, invariance, rxx)
