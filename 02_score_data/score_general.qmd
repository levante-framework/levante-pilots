...
sumscores <- task_data_combined |>
  ...
  mutate(model = NA) |>
  select(item_task, run_id, metric_type, metric_value, model)
...
thetas <- run_data |>
  ...
  left_join(task_map) |>
  mutate(metric_type = "ability (CAT)", model = "roar_irt") |>
  select(item_task, run_id, metric_type,
         metric_value = test_comp_theta_estimate, model)
...
mefs_scores <- read_csv(here("02_scoring_outputs","mefs_data","LEVANTE_20250403_1132.csv")) |>
  ...
  pivot_longer(c(standard_score, total_score),
               names_to = "metric_type", values_to = "metric_value") |>
  mutate(item_task = "mefs", run_id = user_id, language = "en", adaptive = FALSE, model = NA)
```
