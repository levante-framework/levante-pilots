...
```{r}
# foo <- multipleGroup(task_results$data_prepped[[1]], 
#                 itemtype = "Rasch", 
#                 group = task_results$groups[[1]], 
#                 model = mirt.model(task_results$model_str[[1]]), 
#                 verbose = TRUE, 
...
* if CONFIGURAL wins, then we may need to fit separate models
* otherwise refit the best model
```{r}
...
```{r}
write_rds(best_multigroup_refit, 
