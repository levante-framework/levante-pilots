...
task_data_prepped <- task_data_nested |>
  mutate(data = map(data_with_ids, \(df) filter(df, !is.na(item_id))),
         data_filtered = map(data_with_ids, \(df) df |> filter_repeat_runs() |>
=======
task_data_prepped <- task_data_nested |>
...
best_multigroup <- task_multigroup |>
  select(task_id, item_type, groups, mod_metric, mod_scalar, mod_full) |>
...
=======
  select(task_id, model_type, item_type, fscore) |>
  mutate(metric_type = glue("multigroup ({item_type}-{model_type})")) |>
  unnest(fscore) |>
