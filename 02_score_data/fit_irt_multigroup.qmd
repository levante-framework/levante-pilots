...
Download the item match data. 

```{r}
library(googlesheets4)
mrot_items <- read_sheet("https://docs.google.com/spreadsheets/d/1r9taZgU104yjmTxzYjGl_XbFiDKolT2iczx-eCujdH8/edit?gid=241786267#gid=241786267", sheet = "mental-rotation")

mrot_items_mergeable <- mrot_items |>
            unite(item_id_corrected, dims, angle, sep = "_") |>
  select(site, item_id, item_id_corrected) |>
  distinct()

# join in the corrected and merged items
d_mrot_items <- left_join(d_mrot, mrot_items_mergeable)
```
...
task_data_nested_mrot$data[[1]] <- left_join(task_data_nested_mrot$data[[1]], 
                                        mrot_items_mergeable) |>
  select(-item_id) |>
  rename(item_id = item_id_corrected, 
         group = site)
...
mrot_coefs <- task_best$coefs[[1]] |>
  separate(item, into = c("dims","degrees","instance")) |>
  select(-instance) |>
  mutate(degrees = as.numeric(degrees)) 

ggplot(mrot_coefs, aes(x = degrees, y = d, col = dims)) + 
  geom_point() + 
  geom_line()
...
mod_configural <- multipleGroup(task_data_args$task_data_prepped[[2]], 
                                group = task_data_args$groups[[2]], 
                                model = mirt.model(task_data_args$model_str[[2]]), 
                                itemtype = "2PL", 
                                verbose = TRUE)

mod_metric <- multipleGroup(task_data_args$task_data_prepped[[2]], 
                             group = task_data_args$groups[[2]], 
                             model = mirt.model(task_data_args$model_str[[2]]), 
                             itemtype = "2PL", 
                             verbose = TRUE, 
...
mod_scalar1 <- multipleGroup(task_data_args$task_data_prepped[[2]], 
                            group = task_data_args$groups[[2]], 
                            model = mirt.model(task_data_args$model_str[[2]]), 
                            itemtype = "2PL", 
                            verbose = TRUE, 
...
mod_scalar2 <- multipleGroup(task_data_args$task_data_prepped[[2]], 
                            group = task_data_args$groups[[2]], 
                            model = mirt.model(task_data_args$model_str[[2]]), 
                            itemtype = "2PL", 
                            verbose = TRUE, 
...
mod_fullconstrain <- multipleGroup(task_data_args$task_data_prepped[[2]], 
                                   group = task_data_args$groups[[2]], 
                                   model = mirt.model(task_data_args$model_str[[2]]), 
                                   itemtype = "2PL", 
                                   verbose = TRUE, 
