...
```{r}
mrot_1grp_mod <- mirt(mrot$data_prepped[[1]], 
                     itemtype = "Rasch")
mrot_mod <- multipleGroup(mrot$data_prepped[[1]], 
                          group = mrot$groups[[1]],
                     itemtype = "Rasch", 
                     invariance = c("free_means","free_variances", 
                                    "slopes","intercepts"))
coef(mrot_mod, simplify = TRUE)

de_mod <- extract.group(mrot_mod, group = "DE")
co_mod <- extract.group(mrot_mod, group = "CO")

marginal_rxx(mrot_1grp_mod)
marginal_rxx(de_mod) # why are these the same? 
marginal_rxx(co_mod) # why are these the same? 
```
