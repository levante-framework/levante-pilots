...
dataset_names <- c("columbia_pilot_0523_0529:f068:v2_1",
                   ...
                   "columbia_pilot_0604_0605:9g4c:v1_2")
...
```{r}
users <- distinct(dataset_data$users) |>
  left_join(distinct(dataset_data$user_groups), by = "user_id", relationship = "many-to-many") |>
  left_join(distinct(dataset_data$groups), by = "group_id", suffix = c("_user", "_group"))
...
ggsave("plots/ef/hf_ggpairs.png", width = 12, height = 10)
```
...
ggplot(sds_summary, aes(x = age, y = prop_correct)) +
  ...
  theme(legend.position = "bottom")
ggsave("plots/ef/sds_subtasks.png", width = 12, height = 3.5)
...
ggplot(sds_composite, aes(x = age, y = prop_correct)) +
  ...
  labs(x = "Age (years)", y = "Subject's proportion correct responses",
       size = "Number of trials completed") +
  theme(legend.position = "bottom")
ggsave("plots/ef/sds_composite.png", width = 6, height = 5)
```
...
ggplot(mg_summary, aes(x = age, y = longest)) +
  ...
  # theme(legend.position = "bottom")
ggsave("plots/ef/mg_span.png", width = 6, height = 5)
...
ggplot(mg_summary, aes(x = age, y = prop_correct)) +
  ...
  labs(x = "Age (years)", y = "Subject's proportion correct responses",
       size = "Number of correct trials") +
  theme(legend.position = "bottom")
ggsave("plots/ef/mg_accuracy.png", width = 6, height = 5)
```
...
ggsave("plots/ef/ef_pairs.png", width = 11, height = 11)
```

```{r}
# prop_trials <- subject_summary |> select(task, user_id, prop_trials)
