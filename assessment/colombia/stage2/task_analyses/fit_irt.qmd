...
View(egma_no_slide |> group_by(run_id, item_id) |>
    count() |> filter(n>1))

View(egma_no_slide |> filter(is.na(item_id)))
# some issue with number line 4afc...multiple responses for same item?
...
numberline <- egma |>
  ...
  left_join(math_items |> select(item, answer, item_id))
...
# weird...
egma_numberline |> group_by(run_id, item_id) |>
  count() |> filter(n>1)
