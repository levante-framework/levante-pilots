...
# these items can be answered more easily in gendered languages
...
# only in number line 4afc + slider trials
...
sum(is.na(slider_trials$item_id))
...
numline4afc_trials <- egma |> 
  ...
  left_join(math_items |> filter(corpus_trial_type=="number line 4afc") |>
              select(item, answer, item_id, corpus_trial_type))
sum(is.na(numline4afc_trials$item_id))
...
task_data_coded <- task_data |>
  ...
  #bind_rows(sds) |>  
  # TODO: fix SDS chance rate issue, add it back
  bind_rows(egma_numberline) |> bind_rows(tom) |> 
