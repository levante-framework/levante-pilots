...
Compute sumscores, remove irrelevant questions.
...
egma_sumscores |>
  ...
  select(age, addition, `missing number`, multiplication, `number comparison`, `number line 4afc`) |>
...
task_data_coded <- egma_cleaned |>
  mutate(item_id = paste(item,answer, sep=':'),
         chance = ifelse(corpus_trial_type=="number line slider", NUMBERLINE_TOLERANCE*2, .25)) |>
...
```{r}
egma_coefs <- task_results$coefs[[2]] |>
  mutate(item = str_sub(item, 1, -3)) |>
  left_join(tom_items)
