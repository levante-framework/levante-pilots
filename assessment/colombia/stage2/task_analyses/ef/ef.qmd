...
sds_mod_1pl <- mirt(sds_mat, 1, itemtype = "Rasch", 
                verbose = TRUE, 
...
sds_coefs_2pl <- as_tibble(coef(sds_mod_2pl, simplify = TRUE)$items) %>%
  mutate(item = rownames(coef(sds_mod_2pl, simplify = TRUE)$items))
...
```{r}
ano12 <- anova(sds_mod_1pl, sds_mod_2pl)
ano23 <- anova(sds_mod_2pl, sds_mod_3pl)
rbind(ano12, ano23[2,])
```
