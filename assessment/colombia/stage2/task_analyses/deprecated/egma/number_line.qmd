...
trials_numberline <- trials_coded |>
  ...
  select(subtask, item, answer, user_id, run_id, response, correct) |> # assessment_pid had some NAs
...
trials_numberline <- trials_numberline |>
  ...
  mutate(correct_ltn = answer == ltn, # should just use is_correct ?
         #correct_ntl_5 = score_ntl(item, answer, ntl, 0.05)#,
         correct_ntl_10 = score_ntl(item, answer, ntl, 0.1)#,
         #correct_ntl_15 = score_ntl(item, answer, ntl, 0.15)
         ) |>
  select(-ltn, -ntl) |>
  pivot_longer(cols = contains("correct_"), names_to = "metric", values_to = "is_correct")
```

```{r}
by_it <- trials_numberline |>
  mutate(item_label = paste(str_replace(item, "'0': (\\d+), '1': (\\d+)", "\\1-\\2"), answer)) |>
...
trials_numberline |> filter(is.na(correct)) # 12000 trials..many fraction answer, but not all
