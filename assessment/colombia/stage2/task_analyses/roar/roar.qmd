...
roar <- trials_coded |>
  ...
  select(task_id, user_id, run_id, trial_id, item, response, correct, rt, age,
...
roar_thetas <- roar_theta_summary |>
  filter(!(task_id %in% c("sre-es", "vocab"))) |>
...
# roar_scores |> count(task, user_id) |> filter(n > 1)
