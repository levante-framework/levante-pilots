...
```{r}
# slider trials??
# item=="{'0': 0, '1': 10}" / "{'0': 0, '1': 100}" / "{'0': 0, '1': 1000}" / {'0': 0, '1': 1}
#subset(mp, item=="{'0': 0, '1': 10}") # 604
#View(subset(mp, item=="{'0': 0, '1': 100}")) # 346
...
ggplot(coefs_2pl, 
       aes(x = expected_answer, y = d, col = subtask)) + 
  geom_jitter(alpha = .5, width = .5) + 
  geom_smooth(method = "lm")+ 
  facet_wrap(~subtask, scales= "free_y")
```

```{r}
ggplot(coefs_2pl, 
       aes(x = expected_answer, y = a1, col = subtask)) + 
  geom_jitter(alpha = .5, width = .5) + 
  geom_smooth(method = "lm")+ 
  facet_wrap(~subtask, scales= "free_y")
```
...
GGally::ggpairs(ungroup(math_ntr) |> select(-user_id, -run_id))
```
