...
participants <- users |>
  ...
  mutate(dob = ym(paste(birth_year, birth_month, sep = "-")),
         age = as.numeric(difftime(today(), dob, units = "days")) / 365.25) # TODO: age should be based on admin date (last_modified)
...
trials_filtered <- trials_ids |>
  mutate(practice = is_practice | assessment_stage == "practice_response" |
