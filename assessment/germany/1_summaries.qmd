...
```{r}
score_list <- map(list.files("task_analyses/scores", full.names = TRUE), read_rds)
...
ggplot(scores |> filter(!is.na(age)), aes(x = age, y = score)) +
  ...
  geom_smooth(method = "gam", formula = y ~ x, colour = "darkgrey") +
...
ggsave("plots/mxr.png", width = 4, height = 4)

task_plot("mental-rotation")
ggsave("plots/mr.png", width = 4, height = 4)

task_plot(c("theory-of-mind", "hostile-attribution"))
ggsave("plots/tom.png", width = 7, height = 4)
...
ggsave("plots/lang.png", width = 6, height = 6)
```
