...
egma_no_slide <- egma |> 
  ...
  mutate(answer = as.character(ifelse(answer!=corpus_answer, corpus_answer, answer))) |> # correct "7/8" issue
...
task_data_coded <- task_data |>
  ...
  filter(!task_id %in% c("hearts-and-flowers", "egma-math", "same-different-selection", "theory-of-mind", "trog", "vocab")) |>
  bind_rows(hearts_and_flowers) |>
  bind_rows(sds) |> bind_rows(egma_numberline) |>
