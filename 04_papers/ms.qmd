...
table_items <- irt_items %>%
  ...
  select(
    Task = task_id,
    `Total trials (questions)` = total_trials_nonCAT,
    `IRT items (analytic units)` = irt_items
  )

table_items |>
  gt() |>
  tab_header(title = "Number of trials and IRT items per LEVANTE task") |>
  tab_source_note(
    "Note. “Total trials” gives the number of distinct questions"
    %+% "in the non-adaptive version of each task. “IRT items” gives the "
    %+% "number of scored units included in IRT modelling after collapsing "
    %+% "repeated trials and excluding practice/feedback trials."
  )
...
score_counts <- all_scores |> count(site, task_id, name = "n_scores")

# age in range scores (above scores + age in range)
age_score_counts <- age_scores |> count(site, task_id, name = "n_age_scores")

# included scores (above scores + first run or large enough age gap)
included_score_counts <- included_scores |> count(site, task_id, name = "n_included_scores")
...
counts <- list(run_counts, run_nonempty_counts, run_filtered_counts, trial_filtered_run_counts, score_counts, age_score_counts, included_score_counts) |>
  reduce(left_join) |>
...
# write_csv(counts, here("04_papers/display/counts.csv"))
...
site_table <- bind_rows(site_endpoints, site_diffs) |>
  ...
  mutate(count = count |> fct_recode(
    ...
    "Excluded for age out of range" = "scores - age_scores",
    "Excluded for non-retest non-first run or retest with too short retest gap" = "age_scores - included_scores",
...
fixed_only <- coretask_scores %>%
  ...
  filter(!(task == "Memory" & site_label == "uniandes-co")) %>%  # rop CO Memory
  droplevels()

pred_curves_augment <- function(task_name) {
  d <- fixed_only %>% filter(item_task == task_name)
...
#| echo: false
#| results: "hide" 
# vars_core_std$log_age_c <- scale(log(vars_core_std$age))[,1] # removing log_age_c following feedback from BD and NR.
vars_core_std$age_c <- scale(vars_core_std$age, scale = FALSE)[, 1] # center age
...
source(here("03_summaries/modified_sem_plot.R")) # veronica helper
...
ggplot(ability_scores_cat,
       aes(x = metric_value, y = metric_se, colour = adaptive)) +
...
#   summarise(
#     n = n(),
#     mean_se = mean(metric_se, na.rm = TRUE),
#     .groups = "drop"
#   ) %>%
