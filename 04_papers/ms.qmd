...
author: 
  ...
  - name: "Jason D. Yeatman"
    ...
    roles:
      ...
      - visualization
      - writing – review & editing
  - name: "Michael J. Sulik"
    orcid: 0000-0002-4405-6554
    affiliations:
      - ref: stanford
    roles:
      - methodology
...
task_table <- tribble(
  ~Construct, ~`LEVANTE task name`, ~`Prior/Source task name`, ~`Most similar source citation`,
  "Executive Function", "Hearts and Flowers",       "AMES Hearts and Flowers",                              "@davidson2006",
  "",                   "Memory",                   "AMES Memory Game",                                     "@corsi1972",
  "",                   "Same and Different",       "Something’s the Same/Flexible Item Selection Task",    "@willoughby2011",         
  "",                   "",                         "",                                                     "@jacques2001",
  "Language",           "Vocabulary",               "Receptive Vocabulary",                                 "@dunn1965",
  "",                   "Sentence Understanding",   "Test for Reception of Grammar (TROG)",                 "@bishop1982",
  "Math",               "Math",                     "Early Grade Math Assessment (EGMA)",                   "@platas2014",
  "Reasoning",          "Pattern Matching",         "Matrix Reasoning",                                     "@chierchia2019",
  "Spatial Cognition",  "Shape Rotation",           "Mental Rotation",                                      "@frick2013",
  "Social Cognition",   "Stories",                  "Theory of Mind",                                       "@sotomayor-enriquez2024"
)

line_rows <- task_table |>
  mutate(i = 1:n() - 1) |>
  filter(Construct != "") |>
  slice(-1) |>
  pull(i)
...
score_age_cors <- included_mod_scores |>
  group_by(task, dataset, model_set, itemtype, nfact, invariance) |>
  summarise(score_age_cor = cor(ability, age)) |>
  ungroup()

age_mods <- included_mod_scores |>
  nest(scores = -c(task, model_set, itemtype, nfact, invariance)) |>
  mutate(scores = map(scores, \(s) s |> mutate(ability = scale(ability)[,1]))) |>
  mutate(score_age_mod = map(scores, \(s) lm(ability ~ age * dataset, data = s)))
...
age_slopes <- age_mods |>
  ...
  slope_dataset = map(score_age_mod, \(mod) {
    avg_slopes(mod, variable = "age", by = "dataset") |> as_tibble()
  }))

age_slopes_tbl <- full_join(
  age_slopes |> select(-scores, -score_age_mod, -slope_dataset) |> unnest(slope) |> mutate(dataset = "overall"),
  ...
  mutate(invariance = if_else(invariance == "", "configural", invariance),
         itemtype_ = itemtype |> fct_relevel("rasch") |> fct_recode("Rasch" = "rasch", "2PL" = "2pl"),
         site_label = dataset |> fct_recode(!!!site_labels)) |>
  arrange(itemtype_, invariance) |>
  mutate(model = glue("({invariance}) {itemtype_}") |> fct_inorder() |> fct_rev()) |>
  rename(item_task = task) |>
  left_join(task_info) |>
  arrange(task, model)

age_slopes_overall <- age_slopes_tbl |> filter(dataset == "overall")
age_slopes_dataset <- age_slopes_tbl |> filter(dataset != "overall")

# ggplot(age_slopes_dataset, aes(x = estimate, y = model)) +
#   # facet_wrap(vars(task), ncol = 1, scales = "free_y", space = "free_y") +
#   facet_grid(vars(task), vars(site_label), scales = "free_y", space = "free_y") +
...
#   labs(x = "Age slope (SD of ability per year)", y = "Model") +
#   theme(strip.text.y = element_text(size = rel(0.75)),
#         panel.grid.major.y = element_line(colour = "#EBEBEBFF", linewidth = 0.2))
# ggsave("age_slopes.png", width = 14, height = 12)

task_age_slopes <- age_slopes_tbl |> semi_join(task_mods) |>
  arrange(dataset, estimate) |>
  mutate(task = task |> fct_inorder())
task_age_slopes_overall <- task_age_slopes |> filter(dataset == "overall")
task_age_slopes_dataset <- task_age_slopes |> filter(dataset != "overall")

ggplot(task_age_slopes_dataset, aes(x = estimate, y = task)) +
  facet_wrap(vars(site_label), nrow = 1) +
  geom_vline(xintercept = 0, colour = "lightgrey", linetype = "dotted") +
  geom_pointrange(aes(xmin = conf.low, xmax = conf.high, colour = site_label),
                  size = 0.1, alpha = 0.5) +
  geom_pointrange(aes(xmin = conf.low, xmax = conf.high), colour = "black", size = 0.2,
                  data = task_age_slopes_overall) +
  scale_colour_ptol(guide = "none") +
  labs(x = "Age slope (SD of ability per year)", y = "Task", colour = "Model type") +
  theme(panel.grid.major.y = element_line(colour = "#EBEBEBFF", linewidth = 0.2))
# ggsave("age_slopes_task.png", width = 12, height = 3)
```
...
task_time_table <- task_time_summary |>
  ...
  mutate(pct = 100 * as.numeric(adaptive) / as.numeric(non_adaptive),
         non_adaptive = sprintf("%.2f", non_adaptive),
...
![Screenshots from each of the nine LEVANTE core tasks. Top row, from left to right: Sentence Understanding, Vocabulary, Hearts and Flowers; middle row: Memory, Same and Different, Math; bottom row: Pattern Matching, Mental Rotation; Stories.](display/tasks_updated.png){#fig-tasks out-width="100%"}
...
To quantify the age gradients, we fit separate ordinary least-squares models predicting ability scores (IRT units) from centered age with site fixed effects and an age $\times$ site interaction, allowing slopes to vary by site. As summarized in @tbl-slopes, age-related slopes were positive for all tasks and sites, indicating that older children performed better on every measure (see @fig-em-trends in the appendices for visual representation). 

For ease of interpretation, we calculated the zero-order correlations between age and IRT ability scores for each site. As seen in the right side of Table 4, these correlations were uniformly positive ($r = .40$ to .70), of moderate magnitude across tasks, and broadly similar across sites. In sum, the results of the age-based analyses suggest that the tasks are sensitive to age-related differences and are likely suitable for deployment in longitudinal studies of developmental change.

```{r}
#| dependson: load-scores
#| label: fig-age-trends
#| fig-cap: "Estimated IRT ability by age across tasks and sites, non-adaptive data only."
#| fig-width: 8
#| fig-height: 9
...
#   # mutate(site_label = factor(site_label),
#   #        age_c <- scale(age)[,1])
#   nest(task_data = -item_task) |>
#   mutate(age_mod = map(task_data, \(d) lm(score ~ age * site, data = d)))
# 
# age_mods |>
#   mutate(fits = map(age_mod, \(m) augment(m, newdata = grid, se_fit = TRUE)))
# 
# pred_curves_augment <- function(task) {
...
# }
# 
# pred_all <- map_dfr(task_ids, pred_curves_augment) %>%
#   left_join(task_meta, by = "item_task") |>
#   left_join(select(fixed_only, task_category, site_label, site) |> distinct())

dev_change_plot <- ggplot(fixed_only, aes(x = age, y = score, colour = site_label)) +
  ...
  #             aes(y = fit, ymin = lower, 
  #                 ymax = upper, fill = site_label),
  #             alpha = 0.12, colour = NA, show.legend = FALSE) +
  # geom_line(data = pred_all, aes(y= fit, col = site_label), linewidth = 1) + 
  geom_smooth(method = "lm", linewidth = 1) +
  geom_point(alpha = 0.3, size = 1) +
  scale_x_continuous(breaks = seq(6, 14, 2)) +
  .scale_colour_site() +
  guides(colour = guide_legend(
    override.aes = list(shape = NA, linetype = 1, linewidth = 2)
  )) +
  labs(
    x = "Age (years)",
    y = "Ability (IRT score)",
    colour = "Site",
    caption = "Note. Uniandes-co Memory data were excluded due to task bugs and difficulties with the instructions."
  ) +
  theme(
...
# ggsave(
# filename = here::here("04_papers", "display", "dev_change.png"),
# plot = dev_change_plot,
# width = 8,
# height = 9,
# dpi = 300
# )
...
#| include: FALSE
#| eval: FALSE
#| tbl-cap: "Age-Related Change in Performance: Task Grand Means (GM), Site Deviations (Δ), and Age–Ability Correlations ($r$), non-adaptive data only. *Note: for each task, z-scores were regressed on centred age and site, with sum-to-zero contrasts for site. GM β is the grand-mean age slope, Δβ is each site’s deviation from this slope, site-specific slope = GM β + Δβ. $r$ is the within-site Pearson correlation between age and task score.*"
#| ft.arraystretch: 1
...
#   #        age_c <- scale(age)[,1])
#   nest(task_data = -item_task) |>
#   mutate(age_mod = map(task_data, \(d) lm(score ~ age_c * site_label, data = d)))
# 
# age_mods

analyze_task_slopes <- function(tsk) {
  d <- fixed_only %>% filter(item_task == tsk) |> droplevels()
  # d$site  <- as.factor(d$site)
  # d$age_c <- scale(d$age, scale = FALSE)[, 1]
  # contrasts(d$site) <- contr.sum(nlevels(d$site))

  fit <- lm(score ~ age_c * site_label, data = d)
  ...
  tr_site <- emtrends(fit, ~ site_label, var = "age_c")
  dev_tab <- contrast(tr_site, "eff") %>%
    summary(infer = c(TRUE, TRUE)) %>%
    as_tibble() %>%
    transmute(
      # site            = sub("\\s*effect$", "", contrast),
      site_label      = str_extract(contrast, "(?<=\\().*(?=\\))"),
  ...
  corr_tab <- d %>%
    filter(!is.na(age_c), !is.na(score)) %>%
    # mutate(site = as.character(site)) %>%
    group_by(site_label) %>%
    summarise(
      N = n(),
      r = if (n() >= 3 && sd(age_c) > 0 && sd(score) > 0)
            cor(age_c, score) else NA_real_,
      .groups = "drop"
    ) %>%
    mutate(
  ...
  dev_tab %>%
    ...
    left_join(corr_tab, by = "site_label") |>
    relocate(item_task, .before = everything())
}

deviation_raw <- map_dfr(task_ids, analyze_task_slopes) %>%
  left_join(task_info)
  # rename(item_task = task) %>%
  # left_join(task_meta, by = "item_task") %>%
...
ft <- flextable(deviation_print) |>
  set_header_labels(
    ...
    `Deviation from Mean`  = "Δβ",
    `Deviation SE`         = "Δβ SE",
    `Deviation p-value`    = "Δβ p",
    `r (age, ability)`     = "r",
    `r p-value`            = "r p"
  ) |>
  colformat_num(
...
# full_join(
#   mod_stats |> select(task, overlap_bic) |> unnest(overlap_bic),
#   mod_stats |> filter(!by_lang) |> select(task, scalar_rxx) |> unnest(scalar_rxx)
# )
...
# #   filter(model_set %in% c("by_language", "multigroup_dataset")) |>
# #   mutate(invariance = if_else(invariance == "", "configural", invariance)) |>
# #   # group_by(task, model_set, itemtype, nfact) |>
# #   # mutate(bic_diff = BIC - min(BIC))
# #   pivot_wider(names_from = invariance, values_from = BIC, names_prefix = "invariance_") |>
# #   mutate(bic_diff = invariance_configural - invariance_scalar) |>
# #   select(-starts_with("invariance_")) |>
# #   arrange(by_lang, task) |>
# #   pivot_wider(names_from = itemtype, values_from = bic_diff, names_prefix = "dbic_")
# # 
# # task_rxx <- mod_stats |>
# #   mutate(task_rxx = if_else(by_lang, lang_rxx, scalar_rxx)) |>
# #   select(task, by_lang, task_rxx) |>
# #   unnest(task_rxx) |>
# #   filter(model_set %in% c("by_language", "multigroup_dataset"))
#   # semi_join(best_mods)
# 
...
#   pivot_wider(names_from = itemtype, values_from = rxx)
#   # pivot_wider(
#   #   names_from   = site,
#   #   values_from  = rxx,
...
# 
# # get reliability columns
# reliab_cols <- names(trx)[startsWith(names(trx), "Reliability_")]
# 
# ft <- trx |>
#   flextable() |>
#   separate_header() |>
#   align_text_col()
# 
# # reliability < 0.6 in blue
# for (col in reliab_cols) {
#   idx <- which(trx[[col]] < 0.6 & !is.na(trx[[col]]))
#   ft  <- color(ft, i = idx, j = col, color = "blue", part = "body")
# }
# 
# ft <- autofit(ft, add_w = 0, add_h = 0)
# ft <- ft |>
#   add_footer_lines(values = "Note. Cells in blue indicate reliability < .60.") |>
#   fontsize(part = "footer", size = 8) |>
#   italic(part = "footer")
# ft
...
source("modified_sem_plot.R") # veronica helper
...
layout_ids <- as.matrix(read_excel("SEM_layout.xlsx", col_names = FALSE))
...
layout_ef <- as.matrix(read_excel("ef_sem_layout.xlsx", col_names = FALSE))
...
layout_lang <- as.matrix(read_excel("lang_sem_layout.xlsx", col_names = FALSE))
