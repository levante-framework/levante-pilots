...
tom_items <- left_join(tom_coefs, item_raw_avgs, by = c("item" = "item_uid", "site"))
...
ggplot(ha_surveys, aes(x = value, y = action, col = age)) + 
  geom_jitter(alpha = .5, height = .05, width = 0) +
  geom_smooth(method = "lm", aes(group = 1), col = "blue") + 
  facet_wrap(~form_subconstruct)
    
ggplot(ha_surveys, aes(x = value, y = attribution, col = age)) + 
  geom_jitter(alpha = .5, height = .05, width = 0) +
  geom_smooth(method = "lm", aes(group = 1), col = "blue") + 
  facet_wrap(~form_subconstruct)
    
ggplot(ha_surveys, aes(x = value, y = ha_composite, col = age)) + 
  geom_point(alpha = .5) +
  geom_smooth(method = "lm", aes(group = 1), col = "blue") + 
  facet_wrap(~form_subconstruct)
