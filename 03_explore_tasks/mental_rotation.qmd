...
### (2) Angle curves (0°–360° untransformed)
...
### (3) Angle curves (reflected angle – folded across 180°) 
...
### (5) Difficulty by rotation angle (IRT). Note this is all models; 2PL scalar is below

```{r difficulty by angle}
...
#ggsave(
#  filename = here::here("03_explore_tasks", "Graphs", "plot_mrot_diff.png"),
#  plot = plot_mrot_diff,
#  width = 10,
#  height = 6,
#  dpi = 300
...
### (6) Discrimination by rotation angle (IRT). Note this is all models; 2PL scalar is below
...
### (7) Difficulty by rotation angle (2PL scalar)

```{r subset 2pl scalar item params}
# colnames(coefs)
# table(coefs$itemtype)
...
### (8) Discrimination by rotation angle (2PL scalar)
...
ggplot(mrot_prop_correct, aes(x = rt_median, y = correct)) +
  geom_point(alpha = 0.5, size = 1) +
...
# plot
p_irt_accuracy <- ggplot(mrot_speed_ability, aes(x = rt_median, y = ability)) +
...
### (12) Trial type by RT (0°–360° untransformed)

```{r Trial type by RT}
# trial-level RTs (trim + seconds)
mrot_rt_trial <- mrot %>% # from sum scores chunk
  filter(!is.na(rt_numeric), is.finite(rt_numeric)) %>%
  mutate(
    rt_s    = as.numeric(rt_numeric) / 1000,
    correct = as.numeric(correct)
...
p_rt_by_trialtype <- ggplot(
  mrot_items_rt_sum,
  aes(x = angle, y = rt_median_correct,
      group = interaction(item_group, stimulus), colour = stimulus)
) +
  ...
  labs(
    ...
    x = "Rotation angle (°)", y = "Reaction time (s)", colour = "Stimulus"
  )
p_rt_by_trialtype
```

### (13) Rotation angle by RT (reflected angle – folded across 180°) 
