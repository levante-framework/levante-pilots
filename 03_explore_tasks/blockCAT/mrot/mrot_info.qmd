...
param_fits_a <- coef_mrot |>
  # select(itemtype, item_group, item, angle, discrimination) |>
  group_by(itemtype, item_group, angle) |>
  summarise(discrimination = mean(discrimination)) |>
  pivot_wider(names_from = item_group, values_from = discrimination) |>
  # mutate(delta_d = `2d` - `3d`) |>
  # mutate(mean_shift = mean(delta_d, na.rm = TRUE),
  #        shift_2d = `2d` - mean_shift) |>
  mutate(shift_2d = if_else(itemtype == "2PL", `2d` - 0.25, 1),
         g = 0.5) |>
  select(itemtype, angle, discrimination = shift_2d, g)

# param_fits_a <- coef_mrot |>
#   filter(item_group == "2d") |>
#   group_by(itemtype, angle) |>
#   summarise(discrimination = mean(discrimination))
...
### Followup: check where thresholds fall in empirical score distribution 

```{r}
#| label: mrot_score_dist
#| fig-width: 9
#| fig-height: 5
#| out-width: 100%
#| fig.show: hold
...
mrot_scores <- run_data |>
  filter(task_code == "mrot") |>
  select(site, dataset, run_id, age, score, score_se) |>
  filter(!is.na(age), age >= 5, age <= 14) |>
  mutate(site = site |> str_remove("pilot_") |> str_replace_all("_", "-"))
mrot_scores_all <- mrot_scores |> bind_rows(mrot_scores |> mutate(site = "all")) |> mutate(site = fct_relevel(site, "all", after = Inf))

thresholds_2pl <- thresholds |> filter(itemtype == "2PL")

ggplot(mrot_scores_all, aes(x = score)) +
  geom_vline(xintercept = thresholds_2pl$theta,
             colour = "lightgrey", linewidth = 0.3) +
  geom_label(aes(label = theta, x = theta), y = Inf, data = thresholds_2pl,
             colour = "darkgrey", vjust = "inward", size = 2.5,
             label.padding = unit(0.15, "lines"), linewidth = 0.2) +
  geom_density(aes(colour = site), linewidth = 0.5) +
  scale_colour_manual(values = c(ptol_pal()(3), "slategrey")) +
  xlim(range(theta)) +
  scale_y_continuous(expand = expansion(mult = c(0.01, 0.1))) +
  labs(x = "Theta", y = "Density", colour = "") +
  theme(legend.position = "bottom")
```

```{r}
blocks <- bind_rows(
  cross_animal_poly |> select(itemtype, block_threshold = theta) |> mutate(block_index = 2),
  cross_poly_shape |> select(itemtype, block_threshold = theta) |> mutate(block_index = 3)
) |>
  ungroup()

mrot_cat_info <- coef_combined |>
  mutate(angle_str = as.character(angle),
         angle_str = if_else(str_length(angle_str) == 2, paste0("0", angle_str), angle_str),
         item_uid = paste(task_code, item_group, item_entry, angle_str, sep = "_"),
         block_index = case_when(item_group == "3d" ~ 3,
                                 str_detect(item_entry, "polygon") ~ 2,
                                 .default = 1)) |>
  left_join(blocks) |>
  select(itemtype, item_group, block_index, block_threshold, item_uid, item_entry, angle, difficulty, discrimination, chance = g) |>
  arrange(itemtype, block_index)

write_rds(mrot_cat_info, "mrot_cat_info.rds", compress = "gz")
```
