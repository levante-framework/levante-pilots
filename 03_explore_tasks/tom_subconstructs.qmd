...
-   [TOM Airtable
    2](https://airtable.com/appe2p0S3xk4DL2qc/tblnnFrYQfMOFhIF9/viwkKX2qgFMYg5J7Q?blocks=hide)
...
-   [Automated runs through
    tasks](https://crowdin.com/project/levantetranslations/screenshots)

-   [TOM
    graphics/assets](https://console.cloud.google.com/storage/browser/levante-assets-prod/visual/theory-of-mind;tab=objects?pageState=(%22StorageObjectListTable%22:(%22f%22:%22%255B%255D%22))&authuser=0&project=hs-levante-admin-prod&pli=1&prefix=&forceOnObjectsSortingFiltering=false)
...
tom =
  ...
  mutate(correct = as.numeric(correct)) %>%
  # add construct by story
  mutate(tom_construct = case_when(
    str_detect(item_uid, "story(1|7|13)_") ~ "FalseBelief",
    str_detect(item_uid, "story(2|6|8|12|14|18)_") ~ "DiverseBelief_RealityKnown",
    str_detect(item_uid, "story(3|9|15)_") ~ "Interpretation",
    str_detect(item_uid, "story(4|10|16)_") ~ "FalseBelief_MoralJudgment",
    str_detect(item_uid, "story(5|11|17)_") ~ "VPT")) %>%
  # add question type for all items
  mutate(question_type = case_when(
    str_detect(item_uid, "reality_check") ~ "ControlQuestion",
    str_detect(item_uid, "emotion_reasoning") ~ "EmotionQuestion",
    str_detect(item_uid, "tom_story16_deception_reference_(1|2|3|4|5)") ~ "EmotionQuestion",
    str_detect(item_uid, "tom_story4_deception_false_belief_(2|3)") ~ "EmotionQuestion",
    str_detect(item_uid, "tom_story4_deception_reality_check_(1|2|3)") ~ "EmotionQuestion",
               TRUE ~ "TestQuestion"))
...
tom_runs =
  ...
  # filter(question_type == "TestQuestion") %>%
...
# summarize performance by question type
tom_runs_question =
  all_tasks %>%
  filter(task_id == "theory-of-mind") %>%
  # add construct by story
  ...
  mutate(question_type = case_when(
    ...
    str_detect(item_uid, "tom_story4_deception_reality_check_(1|2|3)") ~ "EmotionQuestion",
               TRUE ~ "TestQuestion")) %>%
  group_by(site, user_id, run_id, question_type, chance) %>%
  summarise(correct = mean(correct, na.rm = T),  
            age = first(age),
            n_items  = n_distinct(item_uid),
            .groups  = "drop")

# summarize performance by construct, TEST QUESTIONS ONLY
tom_runs_construct =
  all_tasks %>%
  filter(task_id == "theory-of-mind") %>%
  # add construct by story
  ...
  mutate(question_type = case_when(
    ...
    str_detect(item_uid, "tom_story4_deception_reality_check_(1|2|3)") ~ "EmotionQuestion",
               TRUE ~ "TestQuestion")) %>%
  filter(question_type == "TestQuestion") %>%
  group_by(site, user_id, run_id, tom_construct, chance) %>%
  summarise(correct = mean(correct, na.rm = T),  
            age = first(age),
            n_items  = n_distinct(item_uid),
            .groups  = "drop")
...
# plot
tom_questiontype_scores =
  ggplot(tom_runs_question, aes(x = age, y = correct*100)) +
  geom_point(alpha = .2, size = 1) +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 4), se = T) +
  coord_cartesian(ylim = c(0, 100)) +
  scale_x_continuous(limits = c(5, 14),
                     breaks = seq(5, 14, 1)) +
  facet_wrap(~ chance + question_type, ncol = 3) +
  labs(title = "Accuracy across ages by question type",
       x = "Age (years)",
       y = "% correct",
       caption = "Each point = within-participant item average; trend = GAM per site")
...
# plot
tom_construct_scores =
  ggplot(tom_runs_construct, aes(x = age, y = correct*100)) +
  geom_point(alpha = .2, size = 1) +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 4), se = T) +
  coord_cartesian(ylim = c(0, 100)) +
  scale_x_continuous(limits = c(5, 14),
                     breaks = seq(5, 14, 1)) +
  facet_wrap(~ tom_construct) +
  labs(title = "Accuracy across ages by construct (test questions only)",
       x = "Age (years)",
       y = "% correct",
       caption = "Each point = within-participant item average; trend = GAM per site")
...
### Analysis v2: Updated Grouping

Grouping based on theoretical constructs per
<https://docs.google.com/document/d/1M-VdDFk4qprb_DAfCpv8YYUoEZ4oRzCiqLDMhCriCoc/edit?tab=t.0#heading=h.bw3nu3qda091>

```{r}

tom_fa =
  tom %>%
  select(user_id, run_id, dataset,
         item_uid,
         correct, chance,
         tom_construct, question_type) %>%
  filter(
    # v2: has stories 1-6
    str_detect(item_uid, "story(1|2|3|4|5|6)_")) %>% 
  filter(question_type == "TestQuestion")
...
tom_fa_wide =
  tom_fa %>%
  filter(item_uid != "tom_story18_second_order_false_belief_2") %>%
  group_by(user_id, item_uid) %>%
  slice_tail(n = 1) %>%
  pivot_wider(id_cols = c(user_id, run_id),
              names_from = c(item_uid),
              values_from = correct) %>%
  ungroup() %>%
  select(-c(user_id, run_id))
...
tom_model <- "
  # Latent factors
  FalseBelief =~ tom_story1_reality_known_false_belief + tom_story7_reality_known_false_belief + tom_story13_reality_known_false_belief

  FalseBelief_Moral =~ tom_story4_deception_false_belief_1 + tom_story10_deception_false_belief_1 + tom_story16_deception_false_belief_1

  Interpretation =~ tom_story3_interpretation_false_belief + tom_story9_interpretation_false_belief +  tom_story15_interpretation_false_belief

  DiverseBelief_Reality =~ tom_story2_moral_reasoning_false_belief_1 + tom_story2_moral_reasoning_false_belief_2 +   tom_story2_moral_reasoning_false_belief_3 + tom_story6_second_order_false_belief_1 +  tom_story6_second_order_false_belief_2 + tom_story6_second_order_false_belief_3 +  tom_story6_second_order_false_belief_4 +
tom_story6_second_order_false_belief_5 +  tom_story8_moral_reasoning_false_belief_1 +
tom_story8_moral_reasoning_false_belief_2 +
tom_story8_moral_reasoning_false_belief_3 +
tom_story12_second_order_false_belief_1 +
tom_story12_second_order_false_belief_2 +
tom_story12_second_order_false_belief_3 +
tom_story14_moral_reasoning_false_belief_1 + tom_story14_moral_reasoning_false_belief_2 +
tom_story14_moral_reasoning_false_belief_3 +
tom_story18_second_order_false_belief_1 +
tom_story18_second_order_false_belief_3

  VPT =~ tom_story5_reference_reference + tom_story11_reference_reference + tom_story17_reference_reference

  # Story factors for stories with multiple items under same construct
  story2_factor =~ tom_story2_moral_reasoning_false_belief_1 + tom_story2_moral_reasoning_false_belief_2 +   tom_story2_moral_reasoning_false_belief_3

  story6_factor =~ tom_story6_second_order_false_belief_1 + tom_story6_second_order_false_belief_2 +  tom_story6_second_order_false_belief_3 + tom_story6_second_order_false_belief_4 +  tom_story6_second_order_false_belief_5

  story8_factor =~ tom_story8_moral_reasoning_false_belief_1 + tom_story8_moral_reasoning_false_belief_2 +  tom_story8_moral_reasoning_false_belief_3

  story12_factor =~ tom_story12_second_order_false_belief_1 + tom_story12_second_order_false_belief_2 +  tom_story12_second_order_false_belief_3

  story14_factor =~ tom_story14_moral_reasoning_false_belief_1 + tom_story14_moral_reasoning_false_belief_2 + tom_story14_moral_reasoning_false_belief_3

  story18_factor =~ tom_story18_second_order_false_belief_1 +  tom_story18_second_order_false_belief_3

  # Story factors to each other
  story2_factor ~~ 0*story6_factor + 0*story8_factor + 0*story12_factor + 0*story14_factor + 0*story18_factor
  story6_factor ~~ 0*story8_factor + 0*story12_factor + 0*story14_factor + 0*story18_factor
  story8_factor ~~ 0*story12_factor + 0*story14_factor + 0*story18_factor
  story12_factor ~~ 0*story14_factor + 0*story18_factor
  story14_factor ~~ 0*story18_factor

  DiverseBelief_Reality ~~ 0*story2_factor + 0*story6_factor + 0*story8_factor + 0*story12_factor + 0*story14_factor + 0*story18_factor
"

```

Just v2 (Stories 1-6); most data & includes all constructs:

```{r}

model_v2 <- "
  # Latent Factors
  FalseBelief =~ tom_story1_reality_known_false_belief
  
  DiverseBelief_Reality =~ tom_story2_moral_reasoning_false_belief_1 + 
                           tom_story2_moral_reasoning_false_belief_2 + 
                           tom_story2_moral_reasoning_false_belief_3
  
  Interpretation =~ tom_story3_interpretation_false_belief
  
  FalseBelief_Moral =~ tom_story4_deception_false_belief_1
  
  VPT =~ tom_story5_reference_reference
  
  DiverseBelief_Reality =~ tom_story6_second_order_false_belief_1 + 
  ...
  # Within story
  story2_factor =~ tom_story2_moral_reasoning_false_belief_1 + 
                   tom_story2_moral_reasoning_false_belief_2 + 
                   tom_story2_moral_reasoning_false_belief_3
  
  story6_factor =~ tom_story6_second_order_false_belief_1 + 
                   tom_story6_second_order_false_belief_2 + 
                   tom_story6_second_order_false_belief_3 + 
                   tom_story6_second_order_false_belief_4 + 
                   tom_story6_second_order_false_belief_5
  ...
  DiverseBelief_Reality ~~ 0*story2_factor + 0*story6_factor
  story2_factor ~~ 0*story6_factor
"
...
### Analysis v1: Original Grouping

Grouping per
[https://airtable.com/appe2p0S3xk4DL2qc/shrSnlS1BwADsrBGq/tblnnFrYQfMOFhIF9)](https://airtable.com/appe2p0S3xk4DL2qc/shrSnlS1BwADsrBGq/tblnnFrYQfMOFhIF9):)

```{r}

# item overview: There are 19 items total
