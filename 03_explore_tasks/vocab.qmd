...
# Create age brackets
vocab_binned <- vocab |>
  mutate(age_bracket = cut(age, c(4,7,9,13)))
...
colnames(item_accuracy_by_bracket)
summary(item)

item_accuracy_by_bracket |> 
  group_by(age_bracket) |> 
  
n_kids <- count(item_accuracy_by_bracket)
...
# Note: IRT models are constrained to make each group the same on the items.
