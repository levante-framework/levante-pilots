...
colnames(best_multigroup)
View ("02_scored_data", "irt_outputs", "multigroup_best_outputs.rds")

multigroup_scores <- readRDS(here("02_scoring_outputs", "scores", "scores_multigroup.rds"))
colnames(multigroup_scores)
...
```{r}
# Filter to the egma-math model
egma_model_list <- best_multigroup |> 
  filter(task_id == "egma-math") |> 
  pull(coefs)

egma_model <- egma_model_list[[1]]
glimpse(egma_model)

# Filter for difficulty terms
egma_coefs <- egma_model |> 
  group_by(item) |> 
...
egma_groups <- egma |> 
  select(item_uid, item_group) |> 
  distinct() |> 
  mutate(item = glue("{item_uid}_1"))  # match naming convention

egma_difficulty_grouped <- left_join(egma_coefs, egma_groups, by = "item")
