...
Note: 34% of responses are slower than 5s, yet have high accuracy. 12% are slower than 10s; I'm going to filter trials >20s (2.6% of trials)
...
mod_2pl <- mirt(egma_dat, 1, itemtype='2PL', verbose=TRUE,  technical=list(NCYCLES=2000))
mod_2pl_2f <- mirt(egma_dat, 2, itemtype='2PL', verbose=TRUE,  technical=list(NCYCLES=2000))
mod_2pl_3f <- mirt(egma_dat, 3, itemtype='2PL', verbose=TRUE,  technical=list(NCYCLES=2000))

anova(mod_2pl, mod_2pl_2f) # AIC prefers 2d, BIC prefers 1d
anova(mod_2pl_2f, mod_2pl_3f) # AIC prefers 3d, BIC prefers 1d

# also try CFA with just the subtasks
...
TL = 251-259
TP = 260-265"
mod_bif <- mirt(d_mat, bifact, itemtype = "2PL", technical = list(NCYCLES=2000))
```
