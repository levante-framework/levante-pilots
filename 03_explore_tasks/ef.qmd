...
# 05.05.2025 updated
sds_coefs |> count(block, trial) |>
  arrange(block)

# 'same' is empty

# Check 'same'
same_items <- best_multigroup |>
  filter(task_id == "same-different-selection", term == "difficulty") |>
  filter(str_detect(item, "same"))

same_items |>
  mutate(n_underscores = str_count(item, "_")) |>
  count(n_underscores)


# Create classification - sort into "choice" or "feature"
sds_coefs_clean <- sds_coefs |> 
  mutate(
    trial = ifelse(trial == "", "same_block", trial),
...
sds_choice  <- filter(sds_coefs_clean, trial_type == "choice-based")
...
# de-bug
sds_coefs_clean |> filter(trial_type == "feature-based") |> count(block)
sds_coefs |> filter(block == "same") |>
  count(trial)

sds_coefs |> filter(block == "same")

# choice based
sds_choice <- sds_choice |> 
