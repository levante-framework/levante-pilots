...
registry_scores <- readr::read_rds(here::here("02_scoring_outputs","scores","registry_scores.rds")) |>
  filter(item_task %in% c("hf","mg","sds"))

best_coefs <- readr::read_rds(here::here("02_scoring_outputs","best_coefs.rds")) |>
  filter(task %in% c("hf","mg","sds"))

best_models <- readr::read_rds(here::here("02_scoring_outputs","mods_best.rds")) |>
  filter(task %in% c("hf","mg","sds"))
```
...
ggplot(hf_blocks, aes(x = n, y = correct)) + 
  ...
  facet_grid(site ~ item_group) + ylim(0,1)
```
...
hf_it |>
  ggplot(aes(x = item_group, y = correct, 
             group = interaction(user_id, run_id), color = age)) +
...
# Fixed: used to use old multigroup_scores, now uses registry_scores
multigroup_scores_hf <- registry_scores |>
  filter(item_task == "hf") |>
...
Hearts and flowers - item difficulty by trial type

(Note: best multigroup model is Rasch)
```{r}
...
hf_coefs <- filter(best_coefs, task == "hf") |>
  separate(item, sep = "_", into = c("block","trial","sequence")) |>
  distinct()

hf_trial_type <- hf_coefs |>
  ggplot(aes(x = block, y = -d, col = trial, pch = trial)) +
  geom_point() +
...
ggplot(mg_runs, aes(x = age, y = max_span, col = direction)) + 
  geom_jitter(alpha = .3, height = .1, width = 0) + 
  geom_smooth(method = "lm")+ 
...
```{r}
multigroup_scores_mg <- registry_scores |>
  filter(item_task == "mg") |>
  select(item_task, run_id, metric_type, metric_value, metric_se) |>
  left_join(run_ages)
...
mg_coefs <- best_coefs |>
  filter(task == "mg") |>
  separate(item, sep = "_", into = c("task", "direction", "size", "span")) |>
  mutate(span = as.numeric(str_replace(span, "len", ""))) |>
  select(-task) |>
  distinct()

mg_spans_irt <- mg_coefs |>
  group_by(group, span, size, direction) |>
...
Note that we have used grid3 very little and so it's getting dropped from most analysis. (is this still true?)
...
sds_filtered <- sds |>
  ...
  filter(is.na(rt_numeric) | !slow_rt, rt_numeric > 300) |> # , !str_detect(validation_msg_trial, "fast")
...
```{r}
multigroup_scores_sds <- registry_scores |>
...
```{r}
ef_scores <- filter(registry_scores, 
...
```{r}
#mefs_scores_old <- read_csv(here("02_scoring_outputs","mefs_data","LEVANTE_20250403_1132.csv")) |>
#  janitor::clean_names() |>
#  select(child_id, a1_total_score, a1_standard_score)
  
mefs_scores <- readRDS(here("02_scoring_outputs","scores","scores_mefs.rds")) |>
  select(user_id, metric_type, metric_value) |> # run_id == user_id ?
  group_by(user_id, metric_type) |> slice(1) |> # a few kids had 2 runs?
  pivot_wider(id_cols = user_id, names_from = metric_type, values_from = metric_value) |>
  rename(mefs_total_raw = total_score,
         mefs_standard_raw = standard_score
  ) 
