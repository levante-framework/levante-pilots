...
sds |> filter(item_group!="dimensions", item_group!="same") |>
  select(user_id, timestamp, item_group, item, response, correct) |> 
...
# "med-red-star" -> "med-red-star-1-white"
tmp <- sds |> filter(user_id=="6PxwzwbKSAtyQnpMluJS") |> select(-trial_id, -item_uid) |> 
  filter(item_group!="dimensions", item_group!="same")
tmp |> select(user_id, timestamp, item_group, item, response, correct) |> arrange(timestamp) |> View()
...
sds_trials |>
  count(run_id, item_group, item, trial_index) |>
  filter(n > 1)

# sds_disambig <- sds_trials %>%
#   filter(user_id=="6PxwzwbKSAtyQnpMluJS", item_group=="2match") |>
...
# Nest by trial
sds_nested <- sds_trials |>
  select(user_id, run_id, trial_type, trial_index, item, response, selection, correct) |>
  group_by(user_id, run_id, trial_type, trial_index) |>
  arrange(item) |>
  nest(data = c(item, response, selection, correct)) |>
  ungroup()

# sds_correct_original <- sds_trials |> select(run_id, trial_number)

sds_debug <- sds_nested |> 
  filter(user_id == "6PxwzwbKSAtyQnpMluJS") |> 
  mutate(trial_correct = map2(data, trial_type, function(df, type) {
    ...
    previous <- list()
    df$trial_correct <- map_lgl(df$selection, function(sel) {
      result <- compare_selections(sel, previous, ignore_dims)
      
      # 🔍 Debug output:
      cat("\nSelection:", paste(sel, collapse = ", "),
          "\nPrevious:", paste(sapply(previous, paste, collapse = ","), collapse = " | "),
          "\nIgnore dims:", paste(ignore_dims, collapse = ", "),
          "\nResult:", result, "\n")
...
sds_rescored <- sds_nested |> 
  ...
  # select(-data) |> 
  # unnest(trial_correct)


table(sds_rescored$correct, sds_rescored$trial_correct)
#         FALSE  TRUE
#  FALSE   222  5922
#  TRUE      0 16256

sds_rescored |> select(user_id, timestamp, item_group, trial_index, correct, trial_correct) |> 
  arrange(timestamp) |> View()

sds_final <- sds_parsed |>
  left_join(sds_rescored, by = c("user_id", "run_id", "trial_type", "timestamp"))
...
first_run_ages <- run_ages |>
  group_by(user_id) |>
  arrange(age) |>
  mutate(run = row_number()) |>
  filter(run == 1) |>
  select(site, user_id, age)
...
ef_scores_irt <- ef_scores |>
  ...
  mutate(run = row_number()) |>
  filter(run == 1) |>
  select(site, item_task, metric_value, user_id) |>
  pivot_wider(names_from = "item_task", values_from = "metric_value", 
              id_cols = c("user_id", "site")) |>
  janitor:::clean_names() |>
  left_join(first_run_ages)
# |>
#   mutate(across(hf:sds, ~ scale(.x)[,1]), 
...
ef_scores_irt_mefs <- ef_scores_irt |>
  ungroup() |>
...
```{r}
cfa_model_mefs <-  "
ef =~ hf + mg + sds + mefs_standard
ef ~ age
"

fit <- cfa(cfa_model_mefs, ef_scores_irt_mefs, std.lv=TRUE, missing='fiml')
summary(fit, fit.measures=TRUE, standardize=TRUE)
...
```{r}
# configural invariance
fit1 <- cfa(cfa_model_mefs, ef_scores_irt_mefs, std.lv=TRUE, missing='fiml', 
            group = "site")

# weak invariance
fit2 <- cfa(cfa_model_mefs, ef_scores_irt_mefs, std.lv=TRUE, missing='fiml', 
            group = "site", group.equal = "loadings")

# strong invariance
fit3 <- cfa(cfa_model_mefs, ef_scores_irt_mefs, std.lv=TRUE, missing='fiml', 
            group = "site", group.equal = c("intercepts","loadings"))
