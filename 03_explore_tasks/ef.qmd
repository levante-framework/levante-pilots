...
de_sds <- sds |>
  filter(str_detect(response, "mittel|rote|gelb|blau"))
# why are there German-translated stimulus names???? ...only a few subjects
...
# Nest by trial
trial_nested <- sds_trials |>
  group_by(user_id, run_id, trial_type, trial_index) |>
  arrange(item) |>
  nest(data = c(item, response, selection))


# Score each row based on accumulating previous selections
trial_scored <- trial_nested |>
  mutate(trial_result = map2(data, trial_type, function(df, type) {
    ignore_dims <- case_when(
      #type == "same" ~ c("number", "bgcolor"),
      type == "2match" ~ c("number", "bgcolor"),
      type %in% c("3match", "4match") ~ c("size"),
      TRUE ~ character(0)
    )
...
sds_final <- sds_parsed |>
  left_join(scored_trials, by = c("user_id", "run_id", "trial_type", "trial_index"))
```
