...
tmp <- sds_disambig |> filter(user_id=="6PxwzwbKSAtyQnpMluJS") |> select(-trial_id, -item_uid) |> 
  filter(item_group!="dimensions", item_group!="same")
tmp |> select(user_id, timestamp, item_group, item, response, correct) |> arrange(timestamp)
