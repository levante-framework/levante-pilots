...
ggplot(hf_coefs, aes(x = block, y = value, colour = trial)) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.7) +
  facet_wrap(~site)
  labs(
    title = "IRT difficulty by block and trial type",
    x = "Block",
    y = "IRT difficulty",
    colour = "Trial type"
  )

# Alternative graph by site
ggplot(hf_coefs, aes(x = block, y = value, colour = trial, shape = trial)) +
  geom_point(size = 3) +
