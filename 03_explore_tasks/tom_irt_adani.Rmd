...
First, let's create a dataframe without the problematic items:
...
# grab parameters
item_params_scalar = function(model, model_name = "2pl_scalar") {
  coefs_raw = coef(model, simplify = FALSE)

  # Filter out metadata elements
  group_names = setdiff(names(coefs_raw), c("GroupPars", "means", "cov"))

  # Grab shared difficulty values (same for all groups)
  shared_d = map_dbl(coefs_raw[[group_names[1]]], ~ .x["d"])
  shared_item_names = names(coefs_raw[[group_names[1]]])
  
  # Return one row per item per group
  item_params =
    map_dfr(group_names, function(g) {
      group_coefs = coefs_raw[[g]]
      
      tibble(
        item_uid = names(group_coefs),
        a = map_dbl(group_coefs, ~ .x["a1"]),
        d = shared_d,
        b = -shared_d / map_dbl(group_coefs, ~ .x["a1"]),
        site = g,
        model = model_name
      )
    })
...
item_params_2pl_scalar = item_params_scalar(mod_2pl_scalar)
