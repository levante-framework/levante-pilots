...
# first, filter out erroneous mpieva_de runs
...
# grab parameters
item_params_scalar =
  function(model, model_name = "2pl_scalar", item_names) {
    # fetch group names (sites)
    group_names = model@Data$groupNames
    # store number of items
    num_items = length(item_names)

  # extract shared difficulty parameters
  item_par_objects_group1 =
    model@ParObjects$pars[[1]]@ParObjects$pars[1:num_items]

  d_vals =
    map_dbl(item_par_objects_group1, function(x) {
  ...
  all_params =
    map_dfr(seq_along(group_names), function(g) {
      group_pars =
        model@ParObjects$pars[[g]]@ParObjects$pars[1:num_items]
...
# store in object together with item names

# item names aren't included in model output so let's grab them from our matrix
item_names =
  colnames(response_matrix_downward_extension)

# run function over model output
item_params_2pl_scalar =
  item_params_scalar(mod_2pl_scalar,
...
And then ICC (note: curves are identical across sites so we only see 1 in the plot rather
than 3)
...
# set function
compute_icc_curve =
  function(a, b, item_uid, model, site) {
    tibble(theta = seq(-4, 4, length.out = 200),
           # logistic function
...
# run function over item parameters
icc_curves_nonstinker =
  item_params_2pl_scalar %>%
  select(a, b, item_uid, model, site) %>%
  # ..1 = a, ..2 = b, ..3 = item_uid, ..4 = model, ..5 = site
  pmap_dfr(~ compute_icc_curve(a = ..1,
                               b = ..2,
                               item_uid = ..3,
                               model = ..4,
...
## Polychoric Cor Matrix

```{r}

polychoric_output =
  psych::polychoric(response_matrix_downward_extension)

cor_matrix = polychoric_output$rho
...
ggplot(melted_cor,
  ...
  labs(title = "Polychoric Correlation Matrix",
       x = "Item",
       y = "Item",
       fill = "Correlation") +
...
This throws an error right now - need to fix later

```{r}

# fit statistics
# item_fit_scalar =
#   itemfit(mod_2pl_scalar, na.rm = F,
#           fit_stats = "infit")
# 
# item_params_with_fit =
#   item_params_2pl_scalar %>%
#   left_join(item_fit_scalar %>%
#               rownames_to_column("item_uid"), by = "item_uid")
