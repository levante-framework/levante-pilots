...
all_coefs <- read_rds(here("02_scoring_outputs/all_coefs.rds"))
```
...
matrix_trials <- task_data_nested |>
  filter(item_task == "matrix") |>
  unnest(data) |>
  left_join(run_data |> select(run_id, age)) |>
  filter(age > 1)

matrix_items <- matrix_trials |>
  group_by(item_uid, item_group, item) |>
...
matrix_coefs <- all_coefs |>
  filter(task == "matrix", model_set == "multigroup_site", subset == "all_items",
         invariance == "scalar", itemtype == "rasch") |>
  select(item_uid = item, difficulty) |>
  distinct()
  # separate_wider_delim(item, delim = "_", names = c("task", "set", "d", "mat"), cols_remove = FALSE) |>
  # arrange(value) |>
  # mutate(item_uid = item_uid |> fct_inorder())

matrix_items_coefs <- matrix_coefs |> left_join(matrix_items)

ggplot(matrix_items_coefs, aes(x = difficulty, y = item_uid)) +
  # facet_wrap(vars(set, d), scales = "free") +
  # facet_wrap(vars(invariance, group)) +
  # facet_grid(vars(invariance), vars(group)) +
  geom_crossbar(aes(xmin = difficulty, xmax = difficulty), width = 0.5, middle.linewidth = 1) +
  # geom_point(size = 1) +
  # scale_color_ptol() +
  # scale_y_continuous(limits = ~range(.x, 0)) +
  labs(y = "Item", x = "Difficulty")

matrix_items_coefs |>
  mutate(n_trial_bin = cut(n_trials, breaks = c(0, 10, 100, 1000, 10000))) |>
ggplot(aes(x = prop_correct, y = difficulty, colour = n_trial_bin)) +
  geom_point(size = 0.5) +
  scale_colour_viridis_d(direction = -1) +
  # scale_colour_binned() +
  xlim(c(0, 1)) +
  labs(x = "Proportion correct", y = "Difficulty", colour = "N trials")
ggsave("matrix_coef_fixed.png", width = 8, height = 5)

GGally::ggpairs(matrix_items_coefs, columns = c("n_trials", "prop_correct", "difficulty"))
```
