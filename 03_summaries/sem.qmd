...
view(task_scores$age)
unique(task_scores$age)
...
vars_ind <- c("matrix","mrot","hf","mg","sds","trog","vocab","math","tom")
wide_std <- wide_scores
wide_std[vars_ind] <- lapply(wide_std[vars_ind], function(x) as.numeric(scale(x)))
...
fit_cfg <- sem(cfa_model_age_2, data = wide_std,
