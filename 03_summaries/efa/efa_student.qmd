...
for(formi in 1:nrow(student_tidy_co)){
  ...
  corrplot::corrplot(cor_matrix, method = "color", col = col, type = "upper", order = "original",
          tl.pos = "l", 
          tl.col = "black", 
...
ggsave(here("03_summaries","efa","plots","student_loadings.png"))
```
...
plot_load(loadings_df, student_tidy_co[['form_construct']][[formi]])
ggsave(here("03_summaries","efa","plots","belonging_loadings.png"))
```
...
loadings_df <- loadmax(EFAresult$loadings, subconstruct)
plot_load(loadings_df, student_tidy_co[['form_construct']][[formi]])
ggsave(here("03_summaries","efa","plots","peer_loadings.png"))
```
...
plot_load(loadings_df, student_tidy_co[['form_construct']][[formi]])
ggsave(here("03_summaries","efa","plots","academic_loadings.png"))
```
...
plot_load(loadings_df, student_tidy_co[['form_construct']][[formi]])
ggsave(here("03_summaries","efa","plots","growth_loadings.png"))
```
...
corrplot::corrplot(cor_matrix, method = "color", col = col, type = "upper", order = "original",
        ...
        mar=c(0,0,2,0),
        col.lim = c(-1, 1)) 
ggsave(here("03_summaries","efa","plots","all_corrs.png"))
```
