...
caregiver_filtered <- caregiver_data %>%
  ...
  filter(
    ...
    !(form_construct == "Caregiver Well-Being" & grepl("SelfLifeChanges", variable, ignore.case = TRUE))
  )
...
for (i in seq_len(nrow(caregiver_tidy))) {
  ...
  params_df <- params_df %>%
    filter(!is.na(form_subconstruct)) %>%
    mutate(form_subconstruct = substr(form_subconstruct, 1, 10)) 
  
  if (nrow(params_df) == 0) next
  
  p <-   ggplot(params_df, aes(y = reorder(item, value), x = value, color = site)) +
  geom_segment(aes(x = 0, xend = value, y = reorder(item, value), yend = reorder(item, value)),
               position = position_dodge(width = 0.8), linewidth = 0.4) +
  geom_point(position = position_dodge(width = 0.8), size = 2) +
  facet_grid(form_subconstruct ~ type, scales = "free") +
  theme_classic() +
  labs(
    x = "Estimate",
    y = "Item",
  ...
  #ggsave(here::here(paste0("03_summaries/efa/fig/cfa_", gsub("[/\"']", "_", formi), ".png")), width = 7, height = 8, dpi = 300)
}
