...
times <- task_data |>
  ...
  group_by(run_id, dataset) |>
  summarise(time = mean(timestamp))

ggplot(times, aes(x = time, fill = dataset)) +
  geom_histogram() +
...
time_summary <- task_data |>
  ...
  group_by(site, dataset, item_task, run_id) |>
  summarise(n_trials = n(),
            start = min(timestamp), end = max(timestamp)) |>
  ungroup() |>
  mutate(diff = difftime(end, start, units = "mins")) |>
  left_join(run_data |> select(run_id, is_cat = adaptive)) |>
  filter(n_trials > 3, diff < 120)

# ggplot(time_summary, aes(x = diff)) +
#   facet_wrap(vars(item_task)) +
#   geom_histogram()

ggplot(time_summary,# |> filter(item_task %in% core_tasks),
       aes(x = trials, y = diff, color = dataset)) +
  facet_wrap(vars(item_task), scales = "free") +
  geom_point()
ggsave("durations.png", width = 8, height = 8)

task_time_summary <- time_summary |>
  bind_rows(time_summary |> mutate(site = "all", dataset = "all")) |>
  filter(!is.na(is_cat)) |>
  group_by(site, dataset, item_task, is_cat) |>
...
write_rds(task_time_summary, here("03_summaries","tables","task_time_summary.rds"))
```
