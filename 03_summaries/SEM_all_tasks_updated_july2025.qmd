...
# very low number of complete cases (i.e., all 9 tasks)
...
### complete case: re-estimate EFA with lavaan (ESEM-style, cross-loadings)
...
```{r}
loadings <- parameterEstimates(cfa_fit_3, standardized = TRUE) |>
...
cfa_fit_4 <- cfa(cfa_model_4,
                  data = scores_wide_mat_z,
                  estimator = "MLR",
                  missing = "FIML",
                  rotation = "oblimin")

summary(cfa_fit_4, fit.measures = TRUE, standardized = TRUE)
...
loadings <- parameterEstimates(cfa_fit_4, standardized = TRUE) |>
  dplyr::filter(op == "=~") |>  # Keep only factor loadings
  dplyr::select(lhs, rhs, est = std.all)  # lhs = factor, rhs = indicator
...
# loadings from fiml-imputed model (cfa_fit_3)
loadings_fiml <- parameterEstimates(cfa_fit_3, standardized = TRUE) %>%
  filter(op == "=~") %>%
