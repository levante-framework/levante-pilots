...
lang_lit_scores <- multigroup_scores |>
  filter(task_id %in% c("trog", "vocab"), site != "us_pilot") |>
  select(site, task_id, user_id, run_id, metric_type, metric_value) |>
  left_join(run_ages, by = c("site", "user_id", "run_id"))

dir(here("01_fetched_data"))

# check n's
lang_lit_scores |>
  distinct(site, task_id, user_id) |>
  count(site, task_id, name = "n_users")
...
scores_thetas <- read_rds(here("02_scored_data/scores/scores_thetas.rds"))
glimpse(scores_thetas)

swr <- read_rds(here("02_scored_data/scores/scores_thetas.rds")) |>
...
lang_lit_combined |>
  filter(task_id == "trog") |>
  semi_join(
    lang_lit_combined |> filter(task_id == "vocab"),
    by = "user_id"
  ) |> 
  distinct(user_id) |> 
  count()
...
efa_data |>
  summarise(across(-c(user_id, run_id), ~sum(!is.na(.))))
```
