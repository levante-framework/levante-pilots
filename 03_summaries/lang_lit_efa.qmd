...
```{r}
# raw scores
scores_general <- read_rds(here("02_scoring_outputs/scores/scores_general.rds"))

sre <- scores_general |>
  filter(item_task == "sre")  

n_distinct(sre$user_id) #478

# IRT scores
# Load trial-level SRE data
task_data_combined <- read_rds(here("01_fetched_data/task_data_nested.rds")) |>
  filter(item_task == "sre") |>
  unnest(data)

# Score SRE using guessing-adjusted correct per second (as per ROAR guidelines)
sre <- task_data_combined |>
  group_by(site, item_task, user_id, run_id) |>
  filter(site != "ca_pilot" | timestamp < min(timestamp) + 180) |>
  summarise(
    metric_value = (sum(correct, na.rm = TRUE) - sum(!correct, na.rm = TRUE)) / 180,
    .groups = "drop"
  ) |>
  mutate(
    metric_type = "guessing_adjusted_number_correct",
    task_id = "sre"  # Standardise column name for downstream merging
  )

n_distinct(sre$user_id)
