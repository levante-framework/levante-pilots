...
participant_runs <- participants_nested |>
  unnest(ages) |>
  unnest(groups)
```
...
participants_info <- participant_runs |>
  ...
  select(site, user_id, school, grade, age, sex, teacher_id) |>
...
grade_info <- read_csv(here("03_summaries","score_reports","data_co","grade_info_co.csv")) |>
  ...
  rename(user_id = uid) |>
  select(user_id, grade, year, month) 
...
percentile_summary <- percentile_scores |> 
  ...
  group_by(task_category, task, fct_grade, grade, percentile) |> 
  dplyr::count(.drop = FALSE) |>
  group_by(task_category, task, grade) |>
