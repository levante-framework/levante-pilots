...
```{r semplot, fig.width = 6, fig.height = 6}
...
semPlot::semPaths(fit_1, whatLabels="std",
...
semPlot::semPaths(fit_g, whatLabels="std",
                  residuals=FALSE, intercepts=FALSE,
                  layout="tree")
```

Checking Indices and loadings across both fit_1 and g

```{r}
# compare
fitMeasures(fit_1, c("cfi","tli","rmsea","srmr","chisq","df"))
fitMeasures(fit_g, c("cfi","tli","rmsea","srmr","chisq","df"))

# loadings, fit_1
pe <- parameterEstimates(fit_1, standardized = TRUE)
pe_g <- parameterEstimates(fit_g, standardized = TRUE)

loadings_fit_1 <- pe %>%
  filter(op == "=~") %>%
  transmute(factor = lhs, item = rhs,
            lambda = std.all, p = pvalue)
print(loadings_fit_1)
...
loadings_fit_g <- pe_g %>%
  ...
  transmute(factor = lhs, item = rhs,
            lambda = std.all, p = pvalue)
print(loadings_fit_g)

# Convergent validity (AVE) and Internal consistency (CR) - fit_1
theta_tbl_fit_1 <- pe %>%
  filter(op == "~~", lhs == rhs, rhs %in% vars_core) %>%
  transmute(item = rhs, theta = std.all)

ave_cr_fit_1 <- loadings_fit_1 %>%
  left_join(theta_tbl_fit_1, by = "item") %>%
  group_by(factor) %>%
  summarise(
    n   = n(),
  ...
  mutate(across(c(AVE, CR), ~ round(., 3)))

print(ave_cr_fit_1)     # Rule: AVE ≥ .50, CR ≥ .70 # sds causing ef factor to be lower on both AVE and CR.

# Convergent validity (AVE) and Internal consistency (CR) - fit_g
theta_tbl_g <- pe_g %>%
  filter(op == "~~", lhs == rhs, rhs %in% vars_core) %>%
  transmute(item = rhs, theta = std.all)

ave_cr_g <- loadings_fit_g %>%
  left_join(theta_tbl_g, by = "item") %>%
  group_by(factor) %>%
  summarise(
    n   = n(),
    AVE = mean(lambda^2, na.rm = TRUE),
