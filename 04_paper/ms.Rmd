...
>>>>>>> a987bf9 (updates to validity section)
...
semPlot::semPaths(
  ...
  mar=c(6,6,6,6)
=======
```{r fit for sem, include = FALSE, cache = TRUE}
...
model_1 <- '
  ...
  tom       ~ age
'
fit_1 <- sem(model_1, data = vars_core_std,
...
```{r fig-semplot-main,fig.width=15, fig.height=12}
semPlot::semPaths(
  fit_1,
  what="mod", 
  whatLabels="std",
  ...
  curvePivot = FALSE,      
  # sizes
  sizeMan=8, sizeLat=12, sizeInt=8,
  sizeMan2=6, sizeLat2=6, sizeInt2=1,
  # labels
  label.cex=1, edge.label.cex=1, edge.label.bg=TRUE, nDigits=1,
  nCharNodes = 0,          # don’t truncate node labels 
  mar=c(4,4,4,4)
>>>>>>> a987bf9 (updates to validity section)
)
```

factors are very highly correlated - try with g factor (with age)
...
model_g_no_age <- "
  ...
  # observed outcomes linked to g
  math ~ g
  tom  ~ g"

fit_g <- sem(model_g_no_age, data = vars_core_std,
             estimator = "MLR", missing = "fiml", std.lv = TRUE)
print(summary(fit_g, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE))
>>>>>>> a987bf9 (updates to validity section)
...
```{r fig-semplot-g}
=======
```{r add age to sem, include=FALSE, cache=TRUE}
model_g_age <- paste0(model_g_no_age, "\n g ~ age")
fit_g_age <- sem(model_g_age, data = vars_core_std,
                 estimator = "MLR", missing = "fiml", std.lv = TRUE)
print(summary(fit_g_age, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE))
head(lavaan::modindices(fit_g_age, sort.=TRUE, minimum.value=10), 15)
```
>>>>>>> a987bf9 (updates to validity section)
