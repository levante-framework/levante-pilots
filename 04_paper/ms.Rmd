...
<<<<<<< HEAD
In the second step, we evaluated cross-site comparability using multigroup IRT models. Models were estimated on the set of overlapping items common to all sites, under a sequence of increasingly restrictive invariance constraints. These were defined following the conventional factor-analytic hierarchy but translated into IRT terms:
=======
...
Scalar invariance therefore provides the strongest basis for direct cross-site score comparability, while metric and configural levels reflect progressively weaker comparability.

<<<<<<< HEAD
Results (Table \@ref(tab:model-invariance)) indicated that scalar invariance was tenable for the majority of tasks, suggesting that both item difficulties and slopes generalized across sites. Two tasks supported only metric invariance, implying that discriminations were stable but intercepts varied. Three tasks—Hearts & Flowers, Theory of Mind, and Vocabulary—retained only configural invariance, meaning that while the latent structure was consistent, parameter differences across sites limited strict comparability of scores.

In practice, when a task supported scalar or metric invariance, the corresponding multigroup model was extended to the full item pool and used to generate scores on a common cross-site scale. When only configural invariance was supported, we reverted to site-specific calibrations (the by-language models) for scoring, thereby ensuring valid within-site ability estimates while acknowledging limited comparability across sites.
=======
## Multigroup Calibration and Measurement Invariance 
```{r invariance-summary, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
>>>>>>> c5a41eb (updates to dev trends section)

```{r model-invariance, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
