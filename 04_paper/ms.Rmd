...
task_rxx_table <- task_rxx |>
  ...
  select(task, model_label, site_label, rxx) |>
  pivot_wider(names_from = site_label, values_from = rxx) |>
  arrange(task)

papaja::apa_table(task_rxx_table |> rename(Task = task, `Best model` = model_label),
                  col_spanners = list("Marginal reliability" = c(3,5)))
```
