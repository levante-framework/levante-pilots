...
Model calibration proceeded in two steps. First, within each site and task, alternative unidimensional IRT models were estimated (Rasch vs. 2PL), and the best-fitting specification was identified by the Bayesian Information Criterion (BIC). Task-level model choice was then defined by the specification most consistently favored across sites. As shown in Table \@ref(tab:model-calibration), Rasch models provided the best fit for the majority of tasks, with 2PL models selected only for Same & Different and Shape Rotation. This pattern suggests that a simple Rasch specification adequately captured response variation for most domains, whereas tasks involving more complex perceptual or reasoning processes benefitted from the additional flexibility of the 2PL parameterization. The consistency of model preference across sites further indicates that these solutions are robust rather than site-specific idiosyncrasies.

<<<<<<< HEAD
```{r model-calibration, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
...
>>>>>>> c5a41eb (updates to dev trends section)
...
# Collapse to one model per task
...
# Calibration table (drop invariance)
calibration_tbl <- per_task_model |>
    mutate(Task = recode(task, !!!task_labels)) |>
    select(Task, Model, n_sites) |>
    arrange(Task)

papaja::apa_table(
    calibration_tbl,
    label = "model-calibration",
    caption = "Per-task IRT model selection (Rasch vs. 2PL) based on the lowest Bayesian Information Criterion (BIC) across sites."
)
...
Model calibration proceeded in two steps. First, within each site and task, alternative unidimensional IRT specifications (Rasch vs. 2PL) were estimated, and the best-fitting model was identified using the Bayesian Information Criterion (BIC). Task-level model choice was then defined by the specification most consistently favored across sites. As summarized in Table \@ref(tab:model-invariance), Rasch models provided the best fit for the majority of tasks, whereas 2PL models were favored for Same & Different and Shape Rotation. This pattern suggests that a simple Rasch parameterization was sufficient to capture item response variation for most domains, while tasks involving more complex perceptual or reasoning demands benefitted from the additional flexibility of the 2PL specification. The consistency of model preference across sites supports the robustness of these results rather than site-specific idiosyncrasies.
```{r model-invariance, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
...
# Invariance table (keep invariance column)
invariance_tbl <- per_task_model |>
  mutate(Task = recode(task, !!!task_labels)) |>
  select(Task, Model, invariance, n_sites) |>
  arrange(Task)

papaja::apa_table(
  invariance_tbl,
  label = "model-invariance",
  caption = "Measurement invariance results by task based on multigroup calibration and BIC comparisons."
)
...
In the second step, we evaluated cross-site comparability using multigroup IRT models. These were estimated on the overlapping items common to all sites, under a sequence of increasingly restrictive invariance constraints. Following the conventional factor-analytic hierarchy, translated into IRT terms, we tested:
...
Scalar invariance therefore provides the strongest basis for direct cross-site comparability of latent ability estimates, whereas metric and configural levels reflect progressively weaker comparability.

Results (Table \@ref(tab:model-invariance)) indicated that scalar invariance held for most tasks, suggesting that both item difficulties and slopes generalized across sites. Two tasks supported only metric invariance, implying that discriminations were stable but intercepts varied across groups. Three tasks (Hearts & Flowers, Theory of Mind, Vocabulary) achieved only configural invariance, indicating consistent dimensional structure but insufficient parameter equivalence for strict comparability.

For scoring, we used multigroup models estimated on the full item pool. These models inherited the parameter constraints established in the overlap‐items stage, such that overlapping items retained their fitted parameters while non-overlapping items were freely estimated. This ensured that all scores were placed on a common cross-site scale whenever metric or scalar invariance held. When only configural invariance was supported, we reverted to site-specific (by-language) calibrations of the winning model family, thereby preserving within-site validity while acknowledging weaker cross-site comparability.
...
Across all nine tasks we observe positive age–ability trends. The smoothed lines are roughly parallel within each panel, indicating that sites differ mostly in level with only modest differences in slope (see Figure \@ref(fig:age trends). Growth appears steepest for Math and Hearts & Flowers, moderate for Vocabulary, Memory, and Theory of Mind, and shallower for Shape Rotation; Sentence Understanding shows a positive but weaker trend.
```{r age trends, fig.cap="Age related trends by task", fig.width = 8, fig.height = 8, echo=FALSE, message=FALSE, warning=FALSE}
...
ggplot(coretask_scores, aes(age, metric_value)) +
  ...
  geom_point(aes(colour = site_label), alpha = 0.5, show.legend = FALSE) +  # hide points from legend
...
```
To formally quantify developmental change while treating sites as a random sample from a broader population, we fit linear mixed-effects models with site-specific random intercepts and slopes, which estimate within-site age slopes (IRT units per year) with partial pooling across sites (Figure \@ref(fig:random-slopes-graph)). Consistent with the descriptive smooths, the largest gains were in Math (B=0.61, 95% CI [0.40, 0.82], p=.01) and Hearts & Flowers (B=0.50, [0.09, 0.91], p=.03). Vocabulary (B=0.37, [0.22, 0.52], p=.01), Memory (B=0.36, [0.13, 0.59], p=.02), and Theory of Mind (B=0.33, [0.04, 0.62], p=.04) also showed clear positive change. Sentence Understanding (B≈0.30) and Shape Rotation (B≈0.15) provided weaker evidence (95% CIs include 0). Pattern Matching had a positive point estimate but a wide CI, indicating limited information or instability for that task. See Table \@ref(tab:age-slopes) for per-task estimates.

```{r age slopes, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
slopes_by_task <- coretask_scores %>%
  tidyr::nest(data = -task) %>%                               
  mutate(
    fit   = purrr::map(data, ~ lmer(metric_value ~ age_c + (age_c || site), data = .x)),
    fixed = purrr::map(fit,  ~ broom.mixed::tidy(.x, effects = "fixed", conf.int = TRUE))
  ) %>%
  tidyr::unnest(fixed) %>%
  dplyr::filter(term == "age_c") %>%
  dplyr::select(
    task,
    slope_per_year = estimate,
    ci_low  = conf.low,
    ci_high = conf.high,
    p       = p.value
  ) %>%
  dplyr::arrange(desc(slope_per_year))

slopes_print <- slopes_by_task %>%
  mutate(
    sig = case_when(
      p < .001 ~ "***",
      p < .01  ~ "**",
...
knitr::kable(
  slopes_print, digits = 2,
  caption = "Per-task age slopes (IRT units/year) with 95% CIs; site as random intercept and slope. *p* values from lmerTest. Sig: *p*<.05(*), <.01(**), <.001(***)."
)

```

```{r random slopes, include=FALSE}
# random slopes model
library(lme4)
...
```{r random slopes graph, fig.cap="Random Effects model - Developmental trends", fig.width = 8, fig.height = 8, echo=FALSE, message=FALSE, warning=FALSE}
site_key <- coretask_scores %>% distinct(site, site_label)

age_mean <- mean(coretask_scores$age, na.rm = TRUE)
age_seq  <- seq(min(coretask_scores$age_c, na.rm = TRUE),
                max(coretask_scores$age_c, na.rm = TRUE), length.out = 200)
...
ggplot() +
  geom_point(data = coretask_scores,
             aes(x = age, y = metric_value, colour = site_label),
             alpha = 0.3, size = 1) +
  geom_line(data = newdata,
            aes(x = age, y = y_hat_site, colour = site_label),
            linewidth = 1.1) +
  scale_x_continuous(breaks = seq(6, 14, 2)) +
  .scale_colour_site() +
...
<<<<<<< HEAD
task_rxx <- readr::read_rds(here::here("02_scoring_outputs", "task_rxx.rds"))

rxx_norm <- task_rxx %>%
    mutate(
        invariance = tolower(invariance),
        site_label = recode(site, !!!site_labels)
    )

    rxx_chosen <- rxx_norm %>%
        group_by(task, site_label) %>%
        summarise(rxx = max(rxx, na.rm = TRUE), .groups = "drop")
task_rxx_table <- rxx_chosen %>%
  transmute(Task = recode(task, !!!task_labels),
            site_label, rxx) %>%
  distinct() %>%
  ...
  mutate(across(c(Germany, Colombia, Canada), ~round(.x, 2))) %>%
  select(Task, Germany, Colombia, Canada) %>%
  arrange(Task)
=======
...
papaja::apa_table(
  task_rxx_table,
  col_spanners = list("Marginal reliability" = 2:4),
  caption = "Marginal reliability (rxx) by site for the selected scoring model per task (site-level winners).",
  label   = "marginal-reliability"
)
options(papaja.na_string = "NA")
...
We estimated two models. Model 1 included three latent factors and paths from age to the three factors as well as to math and ToM \@ref(fig:semplot-main). Fit was mixed–CFI = .923, TLI = .850, RMSEA = .084 (90% CI .073–.095), SRMR = .059. However, factor correlations (residual, controlling for age) were high: Reasoning–EF .882, Reasoning–Language .747, and EF–Language .921. This suggests a general ability component could underpin these three latent factors. Controlling for age, maths and ToM showed a small positive residual association (r = .15), suggesting minor overlap not captured by age alone.
...
```{r cfa, main model, include=FALSE}
cfa_1 <- "
...
```{r semplot-main, fig.cap="SEM for core tasks", echo=FALSE, message=FALSE, warning=FALSE}
semPlot::semPaths(
  fit_1, 
  what="mod", 
  whatLabels="std", 
...
```
Given the strong inter-factor correlations, we specified a second model with a hierarchical structure: three first-order factors (Reasoning, EF, Language) loading on a second-order general factor (g) \@ref(fig:fig-semplot-g). Math and ToM were again treated as observed endogenous outcomes regressed on g. We included a single developmental path for age → g Conventional fit indices was similar to Model 1 (CFI = .902, TLI = .857, RMSEA = .081, SRMR = .064). 
...
```{r fig-semplot-g, fig.cap="SEM for core tasks with general factor", echo=FALSE, message=FALSE, warning=FALSE}
...
### External validity
To assess external validity, we fitted a single-factor EF model with hearts and flowers (hf), memory game (mg), same-different-selection (sds), and the external measure, the Minnesota Executive Function Scale (MEFS) as indicators \@ref(fig:semplot-ef). We modeled age as a predictor of the latent EF factor. Fit was generally good (CFI = .971, TLI = .943, SRMR = .034; RMSEA = .071, 90% CI .040–.106). Standardized loadings were moderate to strong (hf = .63, mg = .63, sds = .50, MEFS = .67), indicating that MEFS aligns closely with the latent EF construct. Age strongly predicted EF (β = .79; R² ≈ .62), consistent with expected developmental gains.
```{r prep for sem - ef, include=FALSE}
...
```{r semplot-ef, fig.cap="SEM for Executive Function model", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r fig-semplot-lang, fig.cap="SEM for language model", echo=FALSE, message=FALSE, warning=FALSE}
semPlot::semPaths(
  fit_lang,
  whatLabels = "std",
  residuals  = FALSE,
