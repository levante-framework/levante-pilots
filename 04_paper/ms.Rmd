...
authornote: |
  This work supported by the Jacobs Foundation. 
...
Model calibration proceeded in two steps. First, for each task, alternative unidimensional IRT specifications (Rasch vs. 2PL) were fit within a multigroup framework using the set of items overlapping across sites. The best-fitting model was selected by the Bayesian Information Criterion (BIC), considering both model family (Rasch vs. 2PL) and invariance level (configural, metric, scalar). As summarized in Table \@ref(tab:model-invariance), Rasch models provided the best fit for the majority of tasks, whereas 2PL models were favored for Same & Different and Shape Rotation. This pattern suggests that a simple Rasch parameterization was sufficient to capture response variation in most domains, whereas tasks involving more complex perceptual or reasoning demands benefitted from the additional flexibility of the 2PL specification. The use of a single overlap-based winner per task ensured that selection was driven by fit to the cross-site data rather than majority preference within individual sites.
...
In the second step, we evaluated cross-site comparability by testing measurement invariance in the overlap-based multigroup models. These models were estimated under a sequence of increasingly restrictive constraints, following the conventional factor-analytic hierarchy translated into IRT terms:
=======
...
```{r message=FALSE, warning=FALSE, include=FALSE, eval=FALSE, results='asis'}
...
```{r item-diagnostics, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
...
=======
We estimated two models. Model 1 included three latent factors and paths from age to the three factors as well as to math and ToM \@ref(Dfig:semplot-main). Fit was mixed–CFI = .923, TLI = .850, RMSEA = .084 (90% CI .073–.095), SRMR = .059. However, factor correlations (residual, controlling for age) were high: Reasoning–EF .882, Reasoning–Language .747, and EF–Language .921. This suggests a general ability component could underpin these three latent factors. Controlling for age, maths and ToM showed a small positive residual association (r = .15), suggesting minor overlap not captured by age alone.
>>>>>>> 3b053e5 (text changes)
...
Indicator loadings on their first-order factors were moderate–strong (e.g., matrix reasoning = .79; grammar = .78; vocab = .76), and g strongly underpinned the domains (Reasoning = .91; EF = .99; Language = .89; all standardized). Age positively predicted g (\(\beta = 0.69\); R²(g) = .48). In turn, g strongly predicted math (\(\beta = 0.87\); R² = .76) and moderately predicted ToM (\(\beta = 0.58\); R² = .33), yielding sizable indirect effects of age via g (age → g → Maths = .60; age → g → ToM = .40). After accounting for g, we found a negative residual math–ToM association (r = −.35), indicating that the small positive overlap in Model 1 largely reflected unmodeled general ability.
...
=======
To assess external validity, we fitted a single-factor EF model with hearts and flowers (hf), memory game (mg), same-different-selection (sds), and the external measure, the Minnesota Executive Function Scale (MEFS) as indicators \@ref(fig:semplot-ef). We modeled age as a predictor of the latent EF factor. Fit was generally good (CFI = .971, TLI = .943, SRMR = .034; RMSEA = .071, 90% CI .040–.106). Standardized loadings were moderate to strong (hf = .63, mg = .63, sds = .50, MEFS = .67), indicating that MEFS aligns closely with the latent EF construct. Age strongly predicted EF (\(\beta = 0.79\); R² = .62), consistent with expected developmental gains.
>>>>>>> 3b053e5 (text changes)
...
=======
We ran a similar model to determine the external validity of the language and literacy measures (grammar and vocab) against ROAR tasks including measures of Phonological Awareness (pa), Single Word Reading (swr) and Sentence Reading Efficiency (sre). We fitted a single latent Language factor with ROAR tasks plus grammar (trog) and vocabulary as indicators, and modeled age as a predictor of the latent factor \@ref(fig:fig-semplot-lang). Fit was borderline acceptable: CFI = .937, TLI = .895, RMSEA = .070, 90% CI .052–.089; SRMR = .070. Standardized loadings were: swr = .743, sre = .557, pa = .685, trog = .655, vocab = .814–all significant, indicating that the ROAR tasks align with grammar and vocabulary on a common language/literacy factor, with vocabulary the strongest indicator. Age positively predicted the latent factor (\(\beta = 0.66\); R² = .44), consistent with expected developmental gains.
>>>>>>> 3b053e5 (text changes)
