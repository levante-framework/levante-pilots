...
Across tasks, model selection based on BIC favored Rasch specifications for the majority of domains, with 2PL models selected for Same & Different and Shape Rotation (Table \@ref(tab:model-calibration)). This pattern indicates that a unidimensional Rasch model adequately captured response patterns for most tasks, whereas tasks that involve more complex perceptual or reasoning processes benefitted from the additional flexibility of the 2PL parameterization, where item discriminations are freely estimated. The consistency of model choice across sites (reflected by n_sites) provides evidence that these solutions are robust rather than site-specific idiosyncrasies.
...
To evaluate cross-site comparability, we estimated multigroup IRT models under a sequence of increasingly restrictive invariance constraints. Invariance levels were defined following the conventional factor-analytic hierarchy but translated into IRT terms. Configural invariance corresponds to holding the same factor structure across groups while freely estimating item parameters. Metric invariance in IRT terms constrains item discrimination parameters across groups, while scalar invariance additionally constrains item intercepts (and guessing parameters where applicable). Scalar invariance therefore supports direct cross-site comparability of latent ability estimates, whereas metric and configural levels reflect progressively weaker comparability.

Results (Table \@ref(tab:model-invariance)) indicated that scalar invariance was tenable for the majority of tasks, suggesting that both item difficulties and slopes were comparable across sites. Two tasks achieved only partial fit at the metric level, implying that item discriminations were stable but intercepts varied across groups. Three tasks (Hearts & Flowers, Theory of Mind, Vocabulary) supported only configural invariance, indicating that while the dimensional structure was consistent, differences in item parameters limited strict cross-site score comparability.
