...
```{r}
#| label: tab-slopes
#| caption: "Per-task age correlation (r) and age slopes from OLS with site fixed effects (IRT units per year)" 
...
slopes <- coretask_scores %>%
  ...
  group_modify(~{
    ...
    est <- emtrends(fit, ~ site, var = "age_c") |>
...
final_tab2 <- final_tab %>%
  mutate(
    site = fct_relevel(site, "pilot_western_ca","pilot_unandes_co","pilot_leuphana_de"),
...
knitr::kable(
  final_tab2,
  digits = 3,
  col.names = c("Task","Site","r (age)","Slope / year","CI low","CI high","p-value","N","p (FDR)")
)
