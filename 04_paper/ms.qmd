...
analyze_task_slopes <- function(task_name) {
  ...
  mean_df    <- df.residual(fit)                      # optional, if you want df
...
deviation_print <- deviation_raw %>%
  ...
  transmute(
    ...
    `Grand Mean SE`        = round(Mean_SE, 3),
    `Grand Mean p-value`,
    `Deviation from Mean`  = round(Slope_Deviation, 3),
    `Deviation SE`         = round(SE, 3),
    `Deviation p-value`,
    N,
...
knitr::kable(
  deviation_print,
  format    = "pipe",
  col.names = c("Task","Site","Grand Mean Slope (β)","Grand Mean SE","Grand Mean p-value",
...
knitr::kable(
  trx %>% dplyr::select(sort(names(.))) %>% dplyr::relocate(Task),
  format = "pipe", digits = 3, align = "c"
)

# flextable not working.
#trx |>
