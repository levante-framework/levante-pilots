...
#   ""                  ,  "Same and Different"    ,  "The Same Different Selection Task",   "[@jacques2001]",
#   "Language"          ,  "Vocabulary"            ,  "Picture Vocabulary",                  "[@dunn1965]",
#   ""                  ,  "Sentence Understanding",  "Test for Reception of Grammar (TROG)","[@bishop1982]",
#   "Math"              ,  "Math"                  ,  "Early Grades Math Assessment (EGMA)", "[@platas2014]",
#   "Reasoning"         ,  "Pattern Matching"      ,  "Matrix Reasoning",                    "[@chierchia2019]",
#   ""                  ,  "Shape Rotation"        ,  "Mental Rotation",                     "[@frick2013]",
...
For each task, we compared unidimensional Rasch and two-parameter logistic (2PL) models under these nested invariance constraints. All models were estimated using the `mirt` R package [@mirt] via marginal maximum likelihood using 500 quadrature points, with expected a posteriori (EAP) scoring. Overlapping items across languages served as anchors to establish a common scale; non-overlapping items were freely estimated within the same multi-group calibration. Model convergence and identification were verified by ensuring stable log-likelihoods and Hessian condition numbers below $10^3$.
...
Across all nine tasks, predicted IRT ability increased with age at all sites, indicating robust positive age–ability gradients (see @fig-age-trends). This pattern was particularly pronounced for Vocabulary, Hearts and Flowers, and Math. Age-related slopes were broadly similar across sites: the panels suggest parallel developmental gradients in cognitive performance, with systematic shifts in mean ability but little evidence that the rate of age-related change differs markedly by site.
...
Model fit was imperfect -- CFI: `r round(fitMeasures(fit_cov_age)["cfi"], 3)`, RMSEA: `r round(fitMeasures(fit_cov_age)["rmsea"], 3)` RMSEA p-value: `r pval_fmt(fitMeasures(fit_cov_age)["rmsea.pvalue"])`. In part we expect that lower model fit may be due to a combination of missing data and site-related differences, which are not accounted for in this specification; future work should pursue more complete models that account for these sources of variability.
```{r}
...
Overall, factor loadings supported the intended three-factor structure (convergent validity), and age showed positive associations with each domain. The high inter-factor correlations indicate that much of the variance is shared across domains. 
...
With pilot data collection complete in Canada, Colombia, and Germany, our goal here has been to present the LEVANTE framework tasks, our scoring methods, and an initial analysis of validity and reliability. 
Although task refinement is an iterative process, we discuss the next steps of extending the framework for broader participation in data collection and analysis.
...
```{r}
#| label: fig-em-trends
#| #fig-cap: "Age Slopes by Task, Non-adaptive data only"
#| fig-width: 8
#| fig-height: 9
#| fig-pos: 'H'
