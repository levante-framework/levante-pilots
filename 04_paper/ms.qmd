...
# fig.width = 8, fig.height = 8, echo=FALSE, message=FALSE, warning=FALSE
...
```{r}
#| label: tab-slopes
#| caption: "Per-task age correlation and mixed model age slopes (IRT units/year) with 95% CIs."
...
slopes_print2 <- slopes_print %>%
  ...
  dplyr::relocate(r_age, .after = task) 

# no idea why flextable throws an error here in compilation
knitr::kable(slopes_print2, digits = 3) 
# flextable(slopes_print2) |>
...
To quantify developmental change, we fit linear mixed-effects models with site-specific random intercepts and slopes, which estimate within-site age slopes (IRT units per year) with partial pooling across sites. In addition, we calculated pooled zero-order correlations between age and IRT scores and found that they were  positive and of moderate magnitude across tasks (see @tab-slopes). 
...
Figure @fig-retest shows the relations between time 1 and time 2 scores. Uptake for retest administrations varied between N=`r min(trt$n)` and N=`r max(trt$n)`, likely due to differences in when tasks were sent out and the ordering of tasks in the retest administration. Table @tbl-retest shows test-retest correlations as well as projected reliabilities for the measures based on the Spearman-Brown "prophecy" formula. These reliabilities were lower than the observed marginal reliabilities but generally acceptable, especially considering the substantial delay between administrations and -- in some cases -- the use of alternate items and CAT administration. 
