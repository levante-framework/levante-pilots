...
```{r model-invariance-chunk, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
...
```{r item-diagnostics-chunk, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
...
Across all nine tasks we observed positive age–ability trends. The smoothed lines in Figure \ref{fig:age-trends} are roughly parallel within each panel, indicating that sites differ mostly in level with only modest differences in slope. Growth appears steepest for Math and Hearts & Flowers, moderate for Vocabulary, Memory, and Theory of Mind, and shallower for Shape Rotation; Sentence Understanding shows a positive but weaker trend.
...
To formally quantify developmental change while treating sites as a random sample from a broader population, we fit linear mixed-effects models with site-specific random intercepts and slopes, which estimate within-site age slopes (IRT units per year) with partial pooling across sites. In addition, we calculated pooled zero-order correlations between age and IRT scores and found that they were  positive and of moderate magnitude across tasks (see Table \ref{tab:slopes_print2}). 

```{r age_slopes, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
r_by_task <- coretask_scores %>%
  dplyr::group_by(task) %>%
  dplyr::summarise(
    n      = sum(stats::complete.cases(age, metric_value)),
    r_age  = stats::cor(age, metric_value, use = "pairwise.complete.obs"),
    .groups = "drop"
  )

slopes_print2 <- slopes_print %>%
  dplyr::left_join(r_by_task, by = "task") %>%
  dplyr::relocate(r_age, .after = slope_per_year)

papaja::apa_table(
  slopes_print2,
  digits = 3,
  caption = "(#tab:age_slopes) Per-task age slopes (IRT units/year) with 95% CIs and pooled zero-order correlations (r). *p* values from lmerTest. Sig: *p*<.05(*), <.01(**), <.001(***).")
...
- what is marginal reliability
- which models is it coming from 
- reference table 
...
- Describe retest (how many runs, how many kids, what interval
- Describe which were CAT/not / retest item sets (e.g., go task by task)
- Report retest correlations / reference figure and table [which needs numbers on it]
...
Results (@tbl-model-invariance) indicated that scalar invariance held for most tasks, suggesting that both item difficulties and slopes generalized across sites. Two tasks supported only metric invariance, implying that discriminations were stable but intercepts varied across groups. Three tasks (Hearts & Flowers, Theory of Mind, Vocabulary) achieved only configural invariance, indicating consistent dimensional structure but insufficient parameter equivalence for strict comparability.
...
#     # label cleanup
#     Task       = if (exists("task_labels")) recode(task, !!!task_labels) else task,
#     Model      = recode(itemtype, "rasch" = "Rasch", "2pl" = "2PL", .default = toupper(itemtype)),
#     Invariance = stringr::str_to_title(invariance),
#     Subset     = recode(subset, "all_items" = "All items", "overlap_items" = "Overlap items", .default = subset)
#   ) |>
#   select(
#     Task, Subset, Items = n_items,
#     `Prop flag p`          = prop_flag_p,
...
ggplot(coretask_scores |>
       filter(run_number == 1),
       aes(age, metric_value)) +
  ggh4x::facet_nested_wrap(vars(task_category, task),
                           nest_line = element_line(), solo_line = TRUE,
                           axes = "x", scales = "free_y") +
  geom_smooth(aes(colour = site_label, group = site_label),
...
To formally quantify developmental change while treating sites as a random sample from a broader population, we fit a linear mixed-effects model to scores from each task. Each model contained a fixed effect of age as well as random intercepts and age slopes for each site. In all cases, slopes were positive, though in two cases confidence intervals on the slope estimate included zero (likely due to site-level variation in age gradients for these tasks; e.g. a ceiling effect in sentence understanding for the leuphana-de site). In addition, we calculated pooled zero-order correlations between age and IRT scores and found that they were positive and of moderate magnitude across tasks (see @tbl-age-slopes).
...
# TODO: boundary (singular) fit for several models
slopes_by_task <- coretask_scores |>
  filter(run_number == 1) |>
  nest(data = -task) |>                               
  mutate(fit = map(data, \(f) lmer(metric_value ~ age_c + (age_c | site), 
                                   data = f)),
         fixed = map(fit, \(f) tidy(f, effects = "fixed", conf.int = TRUE))) |>
  unnest(fixed) |>
  filter(term == "age_c") |>
  # select(task, estimate, conf.low, conf.high, p.value) |>
  mutate(across(starts_with("conf"), \(x) round(x, 2))) |>
  mutate(ci = glue("[{conf.low}, {conf.high}]")) |>
  select(task, estimate, ci)
  # rename_with(\(s) str_replace(s, "\\.", "_"))
  # select(task, slope_per_year = estimate, ci_low  = conf.low, ci_high = conf.high, p = p.value) |>
  # arrange(desc(slope_per_year))
...
r_by_task <- coretask_scores |>
  group_by(task) |>
  summarise(n = sum(complete.cases(age, metric_value)),
            r_age = cor(age, metric_value, use = "pairwise.complete.obs")) |>
  ungroup()

slopes_print <- slopes_by_task |>
  left_join(r_by_task, by = "task") |>
  relocate(n, .after = everything())
  # relocate(r_age, .after = estimate)
```

```{r}
#| label: tbl-age-slopes
#| tbl-cap: "Per-task age slopes (IRT units/year) with 95% CIs and pooled zero-order correlations."
# *p* values from lmerTest. Sig: *p*<.05(*), <.01(**), <.001(***).

slopes_print |>
  flextable() |>
  set_header_labels(task = "Task",
                    estimate = "Age slope estimate",
                    ci = "Age slope 95% CI",
                    r_age = "Age correlation",
                    n = "N runs") |>
  flextable::autofit(add_w = 0, add_h = 0)
```
...
IRT models allow the computation of marginal reliability, which captures the interrelatedness of items in an assessment and hence the proportion of variation attributed to shared variance. In multi-group IRT models, marginal reliabilities can be computed only for individual sub-models.  @tbl-task-rxx shows marginal reliabilities for each task, reporting reliability for each sub-model for the best fitting model. 
...
trt |>
  mutate(retest_cat = c("","x","","x","x","","x","","x"), 
         retest_corpus = c("","","","x","","x","x","","")) |>
  flextable() |>
  set_header_labels(task = "Task",
                    retest_cat = "CAT retest",
                    retest_cat = "new items",
                    test_retest_r = "Test-retest correlation",
                    prophecy_r = "Projected reliability",
                    n = "N runs",
                    age_gap = "Mean retest interval (months)") |>
  flextable::autofit(add_w = 0, add_h = 0)
```
...
g_with_age_fit <- fitMeasures(fit_g_age)
# g_with_age_fit
```
