...
Partners at Western University, led by Daniel Ansari, recruited participants from the local community and tested children individually in a clinic setting. The western-ca site collected data from `r user_totals$n_users[user_totals$site == "western-ca"]` children, ages 5-12 years. 
...
```{r}
#| label: tbl-slopes
#| fig-cap: "Age-Related Change in Performance: Task Grand Means, Site Deviations, and Age–Ability Correlations"
...
analyze_task_slopes <- function(task_name) {
  ...
  # combine: slopes + correlations
  dev_tab %>%
    mutate(task = task_name, Mean_Slope = mean_slope, .before = 1) %>%
...
deviation_print <- deviation_raw %>%
  ...
  transmute(
    Task,
    Site = site,
    `Grand Mean Slope (β)` = round(Mean_Slope, 3),
    `Deviation from Mean`  = round(Slope_Deviation, 3),
    SE                     = round(SE, 3),
    `p-value`,
    N,
    `r (age, ability)`     = round(r, 3),
    `r p-value`
  )
...
knitr::kable(
  deviation_print,
  format   = "pipe",
  col.names = c("Task","Site","Grand Mean Slope (β)","Deviation from Mean","SE",
                "p-value","N","r (age, ability)","r p-value"),
...
#| tbl-cap: "Task-wise test-retest correlations computed for Germany data."
 ...
 # with projected reliability computed via the Spearman-Brown \"prophecy\" formula.
...
<!-- as well as projected reliabilities for the measures based on the Spearman-Brown "prophecy" formula.  -->
These reliabilities were lower than the observed marginal reliabilities but generally acceptable, especially considering the substantial delay between administrations and -- in some cases -- the use of alternate items and CAT administration. Lowered reliability in the case of Sentence Understanding likely also reflects the restricted developmental range of the retest sample.
