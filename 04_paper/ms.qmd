...
```{r}
#| label: tbl-slopes
#| tbl-cap: "Per-task age correlations and per-site linear slopes (IRT units/year)."
...
To quantify developmental change, we fitted separate ordinary least-squares models predicting ability scores (IRT units) from centred age with site fixed effects and an age × site interaction, allowing slopes to vary by site. As summarised in Table @tbl-slopes, age-related slopes were positive for all tasks and sites, indicating that older children performed better on every measure. 
