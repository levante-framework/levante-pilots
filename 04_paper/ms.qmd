...
Across all tasks, the sites collected a total of `{r} sum(site_totals)` runs (`{r} get_count_row(1)`) from `{r} sum(user_totals$n_users)` unique children (`{r} user_totals |> deframe() |> collapse_value_list()`), of which some runs were excluded for being incomplete, invalid, or duplicate (`{r} get_count_row(2)`), for missing item metadata (`{r} get_count_row(3)`), for missing age (`{r} get_count_row(4)`), or for age being out of range or having too short (<0.6 months) of a retest gap (`{r} get_count_row(5)`), resulting in a total of `{r} sum(user_score_counts$n_runs)` scored runs (`{r} user_score_counts |> select(site, n_runs) |> deframe() |> collapse_value_list()`) from `{r} sum(user_score_counts$n_users)` unique children (`{r} user_score_counts |> select(site, n_users) |> deframe() |> collapse_value_list()`). 
...
We assessed construct validity using a hierarchical Confirmatory Factor Analysis (CFA) in which three first-order factors—Reasoning (Pattern Matching, Shape Rotation), Executive Function (Hearts & Flowers, Memory, Same & Different), and Language (Sentence Understanding, Vocabulary)—loaded on a second-order general factor (G; @fig-semplot-g). Math and Stories were treated as observed endogenous outcomes because each construct had a single task; introducing one-indicator latent variables would require strong, unverifiable assumptions about reliability/error variances. Age was specified to predict G (age → G). We found that model fit was imperfect -- CFI: `r round(lavaan::fitMeasures(fit_g_age)["cfi"], 3)`, RMSEA: `r round(lavaan::fitMeasures(fit_g_age)["rmsea"], 3)` RMSEA p-value: `r round(lavaan::fitMeasures(fit_g_age)["rmsea.pvalue"], 3)`. In part we expect that lower model fit may be due to a combination of missing data and site-related differences, which are not accounted for in this specificatin; future work should pursue more complete models of the full structure of the data.

Overall, factor loadings supported the intended three-factor structure (convergent validity), and age related to each domain as expected. The high inter-factor correlations and strong second-order loadings indicate that much of the variance is shared across domains, consistent with a broad general ability underpinning performance on the nine core tasks. We expect that exploring this general model within and across sites, ages, and demographic groups will be an exciting locus for future research using the core tasks.
...
name_map <- tibble(
  ...
  pretty = c("Age (log years)","G","Math","Stories","Reasoning","Executive Function","Language",
             "Pattern Matching","Shape Rotation","Hearts & Flowers","Memory","Same & Different","Sentence Understanding","Vocabulary")
)
...
edges <- pe %>%
  ...
  dplyr::filter(!(op == "~~" & ((lhs == "math" & rhs == "tom") |
                                (lhs == "tom"  & rhs == "math")))) %>% # filter out covariance between Math and ToM
  transmute(
