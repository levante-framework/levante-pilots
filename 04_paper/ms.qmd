...
# remove CAT data, remove Columbian data from memory task
fixed_only <- coretask_scores %>% 
  filter(adaptive == FALSE) %>%
  # drop Colombia (uniandes-co) *only* for Memory
  filter(!(task == "Memory" & site_label == "uniandes-co"))
...
g_ef$nodes$fill <- dplyr::case_when(
  ...
  g_ef$nodes$name  == "log_age_c" ~ "#FFF2E0",
  TRUE                         ~ "#F5F5F5"
)
